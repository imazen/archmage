...
impl f32x4 {
    ...
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[f32; 4]) -> Self {
        Self(unsafe { _mm_loadu_ps(data.as_ptr()) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: f32) -> Self {
        Self(unsafe { _mm_set1_ps(v) })
    }

    /// Zero vector (token-gated)
    #[inline(always)]
    pub fn zero(_: archmage::Sse41Token) -> Self {
    ...
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [f32; 4]) -> Self {
    ...
    #[inline(always)]
    pub fn from_bytes_owned(_: archmage::Sse41Token, bytes: [u8; 16]) -> Self {
        // SAFETY: [u8; 16] and Self have identical size
        Self(unsafe { core::mem::transmute(bytes) })
    }
    ...
    pub fn log2_midp(self) -> Self {
        unsafe {
            ...
            let is_zero = _mm_cmp_ps::<_CMP_EQ_OQ>(self.0, zero);
            let is_neg = _mm_cmp_ps::<_CMP_LT_OQ>(self.0, zero);
            let is_inf = _mm_cmp_ps::<_CMP_EQ_OQ>(self.0, _mm_set1_ps(f32::INFINITY));
            ...
            let r = _mm_blendv_ps(result.0, neg_inf, is_zero);
            let r = _mm_blendv_ps(r, nan, is_neg);
            let r = _mm_blendv_ps(r, pos_inf, is_inf);
            let r = _mm_blendv_ps(r, nan, is_nan);
            Self(r)
        }
    }
    ...
    pub fn exp2_midp(self) -> Self {
        unsafe {
            ...
            // Edge case masks
            let is_overflow = _mm_cmp_ps::<_CMP_GE_OQ>(self.0, _mm_set1_ps(128.0));
            let is_underflow = _mm_cmp_ps::<_CMP_LT_OQ>(self.0, _mm_set1_ps(-150.0));
            let is_nan = _mm_cmp_ps::<_CMP_UNORD_Q>(self.0, self.0);
            ...
            let r = _mm_blendv_ps(result.0, pos_inf, is_overflow);
            let r = _mm_blendv_ps(r, zero, is_underflow);
            let r = _mm_blendv_ps(r, nan, is_nan);
            Self(r)
        }
    }
    ...
    pub fn cbrt_midp(self) -> Self {
        unsafe {
            ...
            let is_zero = _mm_cmp_ps::<_CMP_EQ_OQ>(self.0, zero);
            let abs_x = _mm_andnot_ps(_mm_set1_ps(-0.0), self.0);
            let is_inf = _mm_cmp_ps::<_CMP_EQ_OQ>(abs_x, _mm_set1_ps(f32::INFINITY));
            let is_nan = _mm_cmp_ps::<_CMP_UNORD_Q>(self.0, self.0);
...
impl f64x2 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[f64; 2]) -> Self {
        Self(unsafe { _mm_loadu_pd(data.as_ptr()) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: f64) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [f64; 2]) -> Self {
...
impl i8x16 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[i8; 16]) -> Self {
        Self(unsafe { _mm_loadu_si128(data.as_ptr() as *const __m128i) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: i8) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [i8; 16]) -> Self {
...
impl u8x16 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[u8; 16]) -> Self {
        Self(unsafe { _mm_loadu_si128(data.as_ptr() as *const __m128i) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: u8) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [u8; 16]) -> Self {
...
impl i16x8 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[i16; 8]) -> Self {
        Self(unsafe { _mm_loadu_si128(data.as_ptr() as *const __m128i) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: i16) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [i16; 8]) -> Self {
...
impl u16x8 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[u16; 8]) -> Self {
        Self(unsafe { _mm_loadu_si128(data.as_ptr() as *const __m128i) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: u16) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [u16; 8]) -> Self {
...
impl i32x4 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[i32; 4]) -> Self {
        Self(unsafe { _mm_loadu_si128(data.as_ptr() as *const __m128i) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: i32) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [i32; 4]) -> Self {
...
impl u32x4 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[u32; 4]) -> Self {
        Self(unsafe { _mm_loadu_si128(data.as_ptr() as *const __m128i) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: u32) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [u32; 4]) -> Self {
...
impl i64x2 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[i64; 2]) -> Self {
        Self(unsafe { _mm_loadu_si128(data.as_ptr() as *const __m128i) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: i64) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [i64; 2]) -> Self {
...
impl u64x2 {
    ...
    /// Load from array (token-gated)
    #[inline(always)]
    pub fn load(_: archmage::Sse41Token, data: &[u64; 2]) -> Self {
        Self(unsafe { _mm_loadu_si128(data.as_ptr() as *const __m128i) })
    }

    /// Broadcast scalar to all lanes (token-gated)
    #[inline(always)]
    pub fn splat(_: archmage::Sse41Token, v: u64) -> Self {
    ...
    /// This is a zero-cost transmute, not a memory load.
    #[inline(always)]
    pub fn from_array(_: archmage::Sse41Token, arr: [u64; 2]) -> Self {

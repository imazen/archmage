...
impl<T: U8x64Backend> u8x64<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [u8]) -> (&'a [[u8; 64]], &'a [u8]) {
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[u8; 64]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [u8]) -> (&'a mut [[u8; 64]], &'a mut [u8]) {
        let bulk = data.len() / 64;

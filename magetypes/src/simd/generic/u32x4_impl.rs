...
impl<T: U32x4Backend> u32x4<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [u32]) -> (&'a [[u32; 4]], &'a [u32]) {
        let bulk = data.len() / 4;
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[u32; 4]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [u32]) -> (&'a mut [[u32; 4]], &'a mut [u32]) {
        let bulk = data.len() / 4;
        let (head, tail) = data.split_at_mut(bulk * 4);
        // SAFETY: head.len() is bulk * 4, so it's exactly `bulk` chunks of [u32; 4].
        // The pointer cast is valid because [u32] and [[u32; 4]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[u32; 4]>(), bulk)
        };
        (chunks, tail)
    }

...
impl<T: I64x8Backend> i64x8<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [i64]) -> (&'a [[i64; 8]], &'a [i64]) {
        let bulk = data.len() / 8;
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[i64; 8]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [i64]) -> (&'a mut [[i64; 8]], &'a mut [i64]) {
        let bulk = data.len() / 8;

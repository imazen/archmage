...
impl<T: I16x16Backend> i16x16<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [i16]) -> (&'a [[i16; 16]], &'a [i16]) {
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[i16; 16]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [i16]) -> (&'a mut [[i16; 16]], &'a mut [i16]) {
        let bulk = data.len() / 16;

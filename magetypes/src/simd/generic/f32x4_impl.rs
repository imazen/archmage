...
impl<T: F32x4Backend> f32x4<T> {
    ...
    ///
    /// Returns `(&[[f32; 4]], &[f32])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [f32]) -> (&'a [[f32; 4]], &'a [f32]) {
        let bulk = data.len() / 4;
        let (head, tail) = data.split_at(bulk * 4);
        // SAFETY: head.len() is bulk * 4, so it's exactly `bulk` chunks of [f32; 4].
        // The pointer cast is valid because [f32] and [[f32; 4]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[f32; 4]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [f32]) -> (&'a mut [[f32; 4]], &'a mut [f32]) {
        let bulk = data.len() / 4;
        let (head, tail) = data.split_at_mut(bulk * 4);
        // SAFETY: head.len() is bulk * 4, so it's exactly `bulk` chunks of [f32; 4].
        // The pointer cast is valid because [f32] and [[f32; 4]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[f32; 4]>(), bulk)
        };
        (chunks, tail)
    }
...
impl<T: crate::simd::backends::F32x4Convert> f32x4<T> {
    ...
    #[inline(always)]
    pub fn to_i32x4(self) -> super::i32x4<T> {
        self.to_i32()
    }

    /// Alias for [`to_i32_round`](Self::to_i32_round).
    #[inline(always)]
    pub fn to_i32x4_round(self) -> super::i32x4<T> {
        self.to_i32_round()
    }

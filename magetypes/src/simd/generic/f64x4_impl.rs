...
impl<T: F64x4Backend> f64x4<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [f64]) -> (&'a [[f64; 4]], &'a [f64]) {
        let bulk = data.len() / 4;
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[f64; 4]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [f64]) -> (&'a mut [[f64; 4]], &'a mut [f64]) {
        let bulk = data.len() / 4;

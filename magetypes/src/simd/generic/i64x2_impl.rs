...
impl<T: I64x2Backend> i64x2<T> {
    ...
    ///
    /// Returns `(&[[i64; 2]], &[i64])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [i64]) -> (&'a [[i64; 2]], &'a [i64]) {
        let bulk = data.len() / 2;
        let (head, tail) = data.split_at(bulk * 2);
        // SAFETY: head.len() is bulk * 2, so it's exactly `bulk` chunks of [i64; 2].
        // The pointer cast is valid because [i64] and [[i64; 2]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[i64; 2]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [i64]) -> (&'a mut [[i64; 2]], &'a mut [i64]) {
        let bulk = data.len() / 2;
        let (head, tail) = data.split_at_mut(bulk * 2);
        // SAFETY: head.len() is bulk * 2, so it's exactly `bulk` chunks of [i64; 2].
        // The pointer cast is valid because [i64] and [[i64; 2]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[i64; 2]>(), bulk)
        };
        (chunks, tail)
    }

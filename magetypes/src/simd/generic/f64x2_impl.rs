...
impl<T: F64x2Backend> f64x2<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [f64]) -> (&'a [[f64; 2]], &'a [f64]) {
        let bulk = data.len() / 2;
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[f64; 2]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [f64]) -> (&'a mut [[f64; 2]], &'a mut [f64]) {
        let bulk = data.len() / 2;
        let (head, tail) = data.split_at_mut(bulk * 2);
        // SAFETY: head.len() is bulk * 2, so it's exactly `bulk` chunks of [f64; 2].
        // The pointer cast is valid because [f64] and [[f64; 2]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[f64; 2]>(), bulk)
        };
        (chunks, tail)
    }

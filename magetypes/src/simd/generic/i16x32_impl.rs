...
impl<T: I16x32Backend> i16x32<T> {
    ...
    ///
    /// Returns `(&[[i16; 32]], &[i16])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [i16]) -> (&'a [[i16; 32]], &'a [i16]) {
        let bulk = data.len() / 32;
        let (head, tail) = data.split_at(bulk * 32);
        // SAFETY: head.len() is bulk * 32, so it's exactly `bulk` chunks of [i16; 32].
        // The pointer cast is valid because [i16] and [[i16; 32]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[i16; 32]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [i16]) -> (&'a mut [[i16; 32]], &'a mut [i16]) {
        let bulk = data.len() / 32;
...
#[cfg(all(target_arch = "x86_64", feature = "avx512"))]
impl i16x32<archmage::X64V4xToken> {
    /// Implementation identifier for this backend.
    pub const fn implementation_name() -> &'static str {
        "x86::v4x::i16x32"
    }
...
#[cfg(feature = "avx512")]
impl<T: crate::simd::backends::i16x32PopcntBackend> i16x32<T> {

...
impl<T: U16x16Backend> u16x16<T> {
    ...
    ///
    /// Returns `(&[[u16; 16]], &[u16])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice(_: T, data: &[u16]) -> (&[[u16; 16]], &[u16]) {
        let bulk = data.len() / 16;
        let (head, tail) = data.split_at(bulk * 16);
        // SAFETY: head.len() is bulk * 16, so it's exactly `bulk` chunks of [u16; 16].
        // The pointer cast is valid because [u16] and [[u16; 16]] have the same alignment.
        let chunks =
...
impl<T: U16x16Backend> Index<usize> for u16x16<T> {
    ...
    fn index(&self, i: usize) -> &u16 {
        assert!(i < 16, "u16x16 index out of bounds: {i}");
        // SAFETY: u16x16's repr is layout-compatible with [u16; 16], and i < 16.
...
impl<T: crate::simd::backends::I16x16Bitcast> u16x16<T> {
    ...
    pub fn bitcast_ref_i16x16(&self) -> &super::i16x16<T> {
        // SAFETY: u16x16 and i16x16 share the same repr (__m256i / [u16;16] / etc.)
    ...
    pub fn bitcast_mut_i16x16(&mut self) -> &mut super::i16x16<T> {
        // SAFETY: u16x16 and i16x16 share the same repr

...
impl<T: U64x8Backend> u64x8<T> {
    ...
    ///
    /// Returns `(&[[u64; 8]], &[u64])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice(_: T, data: &[u64]) -> (&[[u64; 8]], &[u64]) {
        let bulk = data.len() / 8;
        let (head, tail) = data.split_at(bulk * 8);
        // SAFETY: head.len() is bulk * 8, so it's exactly `bulk` chunks of [u64; 8].
        // The pointer cast is valid because [u64] and [[u64; 8]] have the same alignment.
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut(_: T, data: &mut [u64]) -> (&mut [[u64; 8]], &mut [u64]) {
        let bulk = data.len() / 8;
...
impl<T: U64x8Backend> Index<usize> for u64x8<T> {
    ...
    fn index(&self, i: usize) -> &u64 {
        assert!(i < 8, "u64x8 index out of bounds: {i}");
        // SAFETY: u64x8's repr is layout-compatible with [u64; 8], and i < 8.

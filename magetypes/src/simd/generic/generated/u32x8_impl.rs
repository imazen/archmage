...
impl<T: U32x8Backend> u32x8<T> {
    ...
    ///
    /// Returns `(&[[u32; 8]], &[u32])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice(_: T, data: &[u32]) -> (&[[u32; 8]], &[u32]) {
        let bulk = data.len() / 8;
        let (head, tail) = data.split_at(bulk * 8);
        // SAFETY: head.len() is bulk * 8, so it's exactly `bulk` chunks of [u32; 8].
        // The pointer cast is valid because [u32] and [[u32; 8]] have the same alignment.
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut(_: T, data: &mut [u32]) -> (&mut [[u32; 8]], &mut [u32]) {
        let bulk = data.len() / 8;
...
impl<T: U32x8Backend> Add for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> Sub for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> Mul for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> BitAnd for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> BitOr for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> BitXor for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> AddAssign for u32x8<T> {
    #[inline(always)]
...
impl<T: U32x8Backend> SubAssign for u32x8<T> {
    #[inline(always)]
...
impl<T: U32x8Backend> MulAssign for u32x8<T> {
    #[inline(always)]
...
impl<T: U32x8Backend> BitAndAssign for u32x8<T> {
    #[inline(always)]
...
impl<T: U32x8Backend> BitOrAssign for u32x8<T> {
    #[inline(always)]
...
impl<T: U32x8Backend> BitXorAssign for u32x8<T> {
    #[inline(always)]
...
impl<T: U32x8Backend> Add<u32> for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> Sub<u32> for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> Mul<u32> for u32x8<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: U32x8Backend> Index<usize> for u32x8<T> {
    type Output = u32;
    #[inline(always)]
...
impl<T: U32x8Backend> IndexMut<usize> for u32x8<T> {
    #[inline(always)]
...
impl<T: crate::simd::backends::U32x8Bitcast> u32x8<T> {
    ...
    pub fn bitcast_ref_i32x8(&self) -> &super::i32x8<T> {
        // SAFETY: u32x8 and i32x8 share the same repr (__m256i / [u32;8] / etc.)
    ...
    pub fn bitcast_mut_i32x8(&mut self) -> &mut super::i32x8<T> {
        // SAFETY: u32x8 and i32x8 share the same repr
...
impl u32x8<archmage::ScalarToken> {
...
#[cfg(target_arch = "x86_64")]
impl u32x8<archmage::X64V3Token> {
    /// Implementation identifier for this backend.
    pub const fn implementation_name() -> &'static str {
        "x86::v3::u32x8"
    }
...
impl u32x8<archmage::Wasm128Token> {
    /// Implementation identifier for this backend.
    pub const fn implementation_name() -> &'static str {

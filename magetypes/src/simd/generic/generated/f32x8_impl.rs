...
impl<T: F32x8Backend> f32x8<T> {
    ...
    ///
    /// Returns `(&[[f32; 8]], &[f32])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice(_: T, data: &[f32]) -> (&[[f32; 8]], &[f32]) {
        let bulk = data.len() / 8;
        let (head, tail) = data.split_at(bulk * 8);
        // SAFETY: head.len() is bulk * 8, so it's exactly `bulk` chunks of [f32; 8].
        // The pointer cast is valid because [f32] and [[f32; 8]] have the same alignment.
    ...
    where
        T: crate::simd::backends::F32x8Convert,
    {
        let t = self.1;
...
impl<T: crate::simd::backends::F32x8Convert> f32x8<T> {
    ...
    /// Convert to i32x8 with truncation and UNIFORM saturation:
    ...
    #[inline(always)]
    pub fn to_i32_saturating(self) -> super::i32x8<T> {
        super::i32x8::from_repr_unchecked(self.1, T::convert_f32_to_i32_saturating(self.1, self.0))
    }

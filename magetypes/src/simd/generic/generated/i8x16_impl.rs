//! Generic `i8x16<T>` — 16-lane i8 SIMD vector parameterized by backend.
//!
...
//! The struct delegates all operations to the [`I8x16Backend`] trait.
...
/// 16-lane i8 SIMD vector, generic over backend `T`.
///
...
impl<T: I8x16Backend> i8x16<T> {
    ...
    ///
    /// Returns `(&[[i8; 16]], &[i8])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice(_: T, data: &[i8]) -> (&[[i8; 16]], &[i8]) {
        let bulk = data.len() / 16;
        let (head, tail) = data.split_at(bulk * 16);
        // SAFETY: head.len() is bulk * 16, so it's exactly `bulk` chunks of [i8; 16].
        // The pointer cast is valid because [i8] and [[i8; 16]] have the same alignment.
...
impl<T: I8x16Backend> Index<usize> for i8x16<T> {
    ...
    fn index(&self, i: usize) -> &i8 {
        assert!(i < 16, "i8x16 index out of bounds: {i}");
        // SAFETY: i8x16's repr is layout-compatible with [i8; 16], and i < 16.
...
impl<T: crate::simd::backends::I8x16Bitcast> i8x16<T> {
    ...
    pub fn bitcast_ref_u8x16(&self) -> &super::u8x16<T> {
        // SAFETY: i8x16 and u8x16 share the same repr (__m128i / [i8;16] / etc.)
    ...
    pub fn bitcast_mut_u8x16(&mut self) -> &mut super::u8x16<T> {
        // SAFETY: i8x16 and u8x16 share the same repr
...
impl<T: crate::simd::backends::I8x16Widen> i8x16<T> {
    ...
    pub fn widen_low(self) -> super::i16x8<T> {
        super::i16x8::from_repr_unchecked(self.1, T::widen_low_i8_to_i16(self.1, self.0))
    }

    /// Sign-extend the high half of the lanes to `i16x8`.
    ///
    /// Result lane `i` is `self[i + 8] as i16`.
    #[inline(always)]
    pub fn widen_high(self) -> super::i16x8<T> {
        super::i16x8::from_repr_unchecked(self.1, T::widen_high_i8_to_i16(self.1, self.0))
    }
}

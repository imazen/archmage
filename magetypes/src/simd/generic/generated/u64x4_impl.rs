...
unsafe impl<T: U64x4Backend> crate::simd_storage::TokenStorage for u64x4<T> {
    type Token = T;
}
...
impl<T: U64x4Backend> u64x4<T> {
    ...
    ///
    /// Returns `(&[[u64; 4]], &[u64])` — fixed-size arrays suitable
    ...
    /// Lanes `N..N+4` of the concatenation `[self, hi]` — the
    ...
    /// uses it to slide a window. `N == 0` returns `self`; `N == 4`
...
impl<T: U64x4Backend> Index<usize> for u64x4<T> {
    ...
    fn index(&self, i: usize) -> &u64 {
        &crate::simd_storage::view::<_, [u64; 4]>(&self.0)[i]
    }
}
...
impl<T: U64x4Backend> IndexMut<usize> for u64x4<T> {
    ...
    fn index_mut(&mut self, i: usize) -> &mut u64 {
        &mut crate::simd_storage::view_mut::<_, [u64; 4]>(&mut self.0)[i]
    }
}

...
impl<T: U64x4Backend> u64x4<T> {
    ...
    ///
    /// Returns `(&[[u64; 4]], &[u64])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice(_: T, data: &[u64]) -> (&[[u64; 4]], &[u64]) {
        let bulk = data.len() / 4;
        let (head, tail) = data.split_at(bulk * 4);
        // SAFETY: head.len() is bulk * 4, so it's exactly `bulk` chunks of [u64; 4].
        // The pointer cast is valid because [u64] and [[u64; 4]] have the same alignment.
    ...
    ///
    /// Returns `(&mut [[u64; 4]], &mut [u64])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut(_: T, data: &mut [u64]) -> (&mut [[u64; 4]], &mut [u64]) {
        let bulk = data.len() / 4;
...
impl u64x4<archmage::ScalarToken> {
...
impl u64x4<archmage::Wasm128Token> {
    /// Implementation identifier for this backend.
    pub const fn implementation_name() -> &'static str {

...
impl<T: I8x32Backend> i8x32<T> {
    ...
    /// Lanes `N..N+32` of the concatenation `[self, hi]` — the
    ...
    /// uses it to slide a window. `N == 0` returns `self`; `N == 32`
...
impl<T: I8x32Backend> Index<usize> for i8x32<T> {
    ...
    fn index(&self, i: usize) -> &i8 {
        &crate::simd_storage::view::<_, [i8; 32]>(&self.0)[i]
    }
}
...
impl<T: I8x32Backend> IndexMut<usize> for i8x32<T> {
    ...
    fn index_mut(&mut self, i: usize) -> &mut i8 {
        &mut crate::simd_storage::view_mut::<_, [i8; 32]>(&mut self.0)[i]
    }
}
...
impl<T: crate::simd::backends::I8x32Backend + crate::simd::backends::I16x16Backend> i8x32<T> {
    ...
    pub fn widen_low(self) -> super::i16x16<T> {
        super::i16x16::from_repr_unchecked(
            self.1,
            <T as crate::simd::backends::I8x32Backend>::widen_low_i8_to_i16(self.1, self.0),
        )
    }
    ...
    pub fn widen_high(self) -> super::i16x16<T> {
        super::i16x16::from_repr_unchecked(
            self.1,
            <T as crate::simd::backends::I8x32Backend>::widen_high_i8_to_i16(self.1, self.0),
        )
    }
}

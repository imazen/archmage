//! Generic `i8x32<T>` — 32-lane i8 SIMD vector parameterized by backend.
//!
...
//! The struct delegates all operations to the [`I8x32Backend`] trait.
...
use crate::simd::backends::I8x32Backend;
...
impl<T: I8x32Backend> i8x32<T> {
    ...
    pub fn partition_slice(_: T, data: &[i8]) -> (&[[i8; 32]], &[i8]) {
        ...
        // The pointer cast is valid because [i8] and [[i8; 32]] have the same alignment.
...
impl<T: I8x32Backend> Add for i8x32<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: I8x32Backend> Sub for i8x32<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: I8x32Backend> Neg for i8x32<T> {
    type Output = Self;
    #[inline(always)]
    fn neg(self) -> Self {
...
impl<T: I8x32Backend> BitAnd for i8x32<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: I8x32Backend> BitOr for i8x32<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: I8x32Backend> BitXor for i8x32<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: I8x32Backend> AddAssign for i8x32<T> {
    #[inline(always)]
...
impl<T: I8x32Backend> SubAssign for i8x32<T> {
    #[inline(always)]
...
impl<T: I8x32Backend> BitAndAssign for i8x32<T> {
    #[inline(always)]
...
impl<T: I8x32Backend> BitOrAssign for i8x32<T> {
    #[inline(always)]
...
impl<T: I8x32Backend> BitXorAssign for i8x32<T> {
    #[inline(always)]
...
impl<T: I8x32Backend> Add<i8> for i8x32<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: I8x32Backend> Sub<i8> for i8x32<T> {
    type Output = Self;
    #[inline(always)]
...
impl<T: I8x32Backend> Index<usize> for i8x32<T> {
    type Output = i8;
    #[inline(always)]
    fn index(&self, i: usize) -> &i8 {
        assert!(i < 32, "i8x32 index out of bounds: {i}");
        // SAFETY: i8x32's repr is layout-compatible with [i8; 32], and i < 32.
        unsafe { &*(core::ptr::from_ref(self).cast::<i8>()).add(i) }
    }
}

impl<T: I8x32Backend> IndexMut<usize> for i8x32<T> {
    #[inline(always)]
...
impl<T: crate::simd::backends::I8x32Bitcast> i8x32<T> {
...
// ============================================================================
// Widening (i8x32 -> i16x16)
// ============================================================================

impl<T: crate::simd::backends::I8x32Widen> i8x32<T> {
    /// Sign-extend the low half of the lanes to `i16x16`.
    ///
    /// Result lane `i` is `self[i] as i16` for `i` in `0..16`.
    /// One instruction on every backend, in natural lane order —
    /// see `docs/CROSS-ISA-INT-PRIMITIVES.md`.
    #[inline(always)]
    pub fn widen_low(self) -> super::i16x16<T> {
        super::i16x16::from_repr_unchecked(self.1, T::widen_low_i8_to_i16(self.1, self.0))
    }

    /// Sign-extend the high half of the lanes to `i16x16`.
    ///
    /// Result lane `i` is `self[i + 16] as i16`.
    #[inline(always)]
    pub fn widen_high(self) -> super::i16x16<T> {
        super::i16x16::from_repr_unchecked(self.1, T::widen_high_i8_to_i16(self.1, self.0))
    }
}
...
impl i8x32<archmage::ScalarToken> {
...
#[cfg(target_arch = "x86_64")]
impl i8x32<archmage::X64V3Token> {
    /// Implementation identifier for this backend.
    pub const fn implementation_name() -> &'static str {
        "x86::v3::i8x32"
    }
...
impl i8x32<archmage::Wasm128Token> {
    /// Implementation identifier for this backend.
    pub const fn implementation_name() -> &'static str {

...
impl<T: F32x8Backend> f32x8<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [f32]) -> (&'a [[f32; 8]], &'a [f32]) {
        let bulk = data.len() / 8;
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[f32; 8]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [f32]) -> (&'a mut [[f32; 8]], &'a mut [f32]) {
        let bulk = data.len() / 8;

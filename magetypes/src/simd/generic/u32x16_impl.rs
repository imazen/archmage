//! Generic `u32x16<T>` — 16-lane u32 SIMD vector parameterized by backend.
//!
//! `T` is a token type (e.g., `X64V4Token`, `ScalarToken`)
//! that determines the platform-native representation and intrinsics used.
//! The struct delegates all operations to the [`U32x16Backend`] trait.
...
use crate::simd::backends::U32x16Backend;

/// 16-lane u32 SIMD vector, generic over backend `T`.
///
...
impl<T: U32x16Backend> u32x16<T> {
    ...
    ///
    /// Returns `(&[[u32; 16]], &[u32])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [u32]) -> (&'a [[u32; 16]], &'a [u32]) {
        let bulk = data.len() / 16;
        let (head, tail) = data.split_at(bulk * 16);
        // SAFETY: head.len() is bulk * 16, so it's exactly `bulk` chunks of [u32; 16].
        // The pointer cast is valid because [u32] and [[u32; 16]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[u32; 16]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [u32]) -> (&'a mut [[u32; 16]], &'a mut [u32]) {
        let bulk = data.len() / 16;
...
impl<T: U32x16Backend> Index<usize> for u32x16<T> {
    ...
    fn index(&self, i: usize) -> &u32 {
        assert!(i < 16, "u32x16 index out of bounds: {i}");
        // SAFETY: u32x16's repr is layout-compatible with [u32; 16], and i < 16.

...
impl<T: I32x4Backend> i32x4<T> {
    ...
    ///
    /// Returns `(&[[i32; 4]], &[i32])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [i32]) -> (&'a [[i32; 4]], &'a [i32]) {
        let bulk = data.len() / 4;
        let (head, tail) = data.split_at(bulk * 4);
        // SAFETY: head.len() is bulk * 4, so it's exactly `bulk` chunks of [i32; 4].
        // The pointer cast is valid because [i32] and [[i32; 4]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[i32; 4]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [i32]) -> (&'a mut [[i32; 4]], &'a mut [i32]) {
        let bulk = data.len() / 4;
        let (head, tail) = data.split_at_mut(bulk * 4);
        // SAFETY: head.len() is bulk * 4, so it's exactly `bulk` chunks of [i32; 4].
        // The pointer cast is valid because [i32] and [[i32; 4]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[i32; 4]>(), bulk)
        };
        (chunks, tail)
    }
...
impl<T: I32x4Backend> Index<usize> for i32x4<T> {
    ...
    fn index(&self, i: usize) -> &i32 {
        assert!(i < 4, "i32x4 index out of bounds: {i}");
        // SAFETY: i32x4's repr is layout-compatible with [i32; 4], and i < 4.

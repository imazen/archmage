...
impl<T: U16x16Backend> u16x16<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [u16]) -> (&'a [[u16; 16]], &'a [u16]) {
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[u16; 16]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [u16]) -> (&'a mut [[u16; 16]], &'a mut [u16]) {
        let bulk = data.len() / 16;

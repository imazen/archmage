...
impl<T: U16x8Backend> u16x8<T> {
    ...
    ///
    /// Returns `(&[[u16; 8]], &[u16])` — the bulk portion reinterpreted
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [u16]) -> (&'a [[u16; 8]], &'a [u16]) {
        let bulk = data.len() / 8;
        let (head, tail) = data.split_at(bulk * 8);
        // SAFETY: head.len() is bulk * 8, so it's exactly `bulk` chunks of [u16; 8].
        // The pointer cast is valid because [u16] and [[u16; 8]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[u16; 8]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [u16]) -> (&'a mut [[u16; 8]], &'a mut [u16]) {
        let bulk = data.len() / 8;
        let (head, tail) = data.split_at_mut(bulk * 8);
        // SAFETY: head.len() is bulk * 8, so it's exactly `bulk` chunks of [u16; 8].
        // The pointer cast is valid because [u16] and [[u16; 8]] have the same alignment.
        let chunks = unsafe {
            core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[u16; 8]>(), bulk)
        };
        (chunks, tail)
    }
...
impl<T: U16x8Backend> Index<usize> for u16x8<T> {
    ...
    fn index(&self, i: usize) -> &u16 {
        ...
        // SAFETY: u16x8's repr is layout-compatible with [u16; 8], and i < 8.
...
impl<T: crate::simd::backends::I16x8Bitcast> u16x8<T> {
    ...
    pub fn bitcast_ref_i16x8(&self) -> &super::i16x8<T> {
        // SAFETY: u16x8 and i16x8 share the same repr (__m128i / [u16;8] / etc.)
    ...
    pub fn bitcast_mut_i16x8(&mut self) -> &mut super::i16x8<T> {
        // SAFETY: u16x8 and i16x8 share the same repr

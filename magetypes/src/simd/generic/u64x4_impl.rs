...
impl<T: U64x4Backend> u64x4<T> {
    ...
    #[inline(always)]
    pub fn partition_slice<'a>(_: T, data: &'a [u64]) -> (&'a [[u64; 4]], &'a [u64]) {
        let bulk = data.len() / 4;
        ...
        let chunks = unsafe {
            core::slice::from_raw_parts(head.as_ptr().cast::<[u64; 4]>(), bulk)
        };
        (chunks, tail)
    }
    ...
    /// as fixed-size arrays suitable for [`load`](Self::load), plus any leftover elements.
    #[inline(always)]
    pub fn partition_slice_mut<'a>(_: T, data: &'a mut [u64]) -> (&'a mut [[u64; 4]], &'a mut [u64]) {
        let bulk = data.len() / 4;

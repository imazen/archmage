...
impl f32x4 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[f32]) -> (&[[f32; 4]], &[f32]) {
        let bulk = data.len() / 4;
    ...
    pub fn pow_lowp(self, n: f32) -> Self {
        unsafe {
            let result = Self(_mm_mul_ps(self.log2_lowp().0, _mm_set1_ps(n))).exp2_lowp();
            // Zero masking: pow(0, n) = 0 for n > 0
            let is_zero = _mm_cmp_ps::<_CMP_EQ_OQ>(self.0, _mm_setzero_ps());
            let result = _mm_blendv_ps(result.0, _mm_setzero_ps(), is_zero);
            Self(result)
        }
    }
    ...
    pub fn exp2_midp_unchecked(self) -> Self {
        ...
        unsafe {
            ...
            let xi = _mm_min_ps(_mm_round_ps::<0x08>(x), _mm_set1_ps(127.0));
            let xf = _mm_sub_ps(x, xi);
    ...
    pub fn exp2_midp(self) -> Self {
        unsafe {
            ...
            let x_clamped = _mm_max_ps(x, _mm_set1_ps(-126.0));
            ...
            // Handle edge cases
            let underflow = _mm_cmp_ps::<_CMP_LT_OQ>(x, _mm_set1_ps(-126.0));
            let overflow = _mm_cmp_ps::<_CMP_GE_OQ>(x, _mm_set1_ps(128.0));
...
impl f64x2 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[f64]) -> (&[[f64; 2]], &[f64]) {
        let bulk = data.len() / 2;
    ...
    /// Create from raw `__m128d` (token-gated, zero-cost).
    ///
    /// This is the safe alternative to [`from_raw`](Self::from_raw). The token
    /// proves the CPU supports the required SIMD features.
    #[inline(always)]
    pub fn from_m128d(_: archmage::X64V3Token, v: __m128d) -> Self {
        Self(v)
    }

    /// Create from raw intrinsic type (token-gated, zero-cost).
    ///
    /// Portable alias for [`from_m128d`](Self::from_m128d).
    /// Use this when you want architecture-independent code.
    #[inline(always)]
    pub fn from_raw_checked(_: archmage::X64V3Token, v: __m128d) -> Self {
        Self(v)
    }
...
impl i8x16 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[i8]) -> (&[[i8; 16]], &[i8]) {
    ...
    /// Create from raw `__m128i` (token-gated, zero-cost).
    ///
    /// This is the safe alternative to [`from_raw`](Self::from_raw). The token
    /// proves the CPU supports the required SIMD features.
    #[inline(always)]
    pub fn from_m128i(_: archmage::X64V3Token, v: __m128i) -> Self {
        Self(v)
    }

    /// Create from raw intrinsic type (token-gated, zero-cost).
    ///
    /// Portable alias for [`from_m128i`](Self::from_m128i).
    /// Use this when you want architecture-independent code.
    #[inline(always)]
    pub fn from_raw_checked(_: archmage::X64V3Token, v: __m128i) -> Self {
        Self(v)
    }
...
impl u8x16 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[u8]) -> (&[[u8; 16]], &[u8]) {
        let bulk = data.len() / 16;
        let (head, tail) = data.split_at(bulk * 16);
...
impl i16x8 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[i16]) -> (&[[i16; 8]], &[i16]) {
        let bulk = data.len() / 8;
...
impl u16x8 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[u16]) -> (&[[u16; 8]], &[u16]) {
        let bulk = data.len() / 8;
        let (head, tail) = data.split_at(bulk * 8);
...
impl i32x4 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[i32]) -> (&[[i32; 4]], &[i32]) {
        let bulk = data.len() / 4;
        let (head, tail) = data.split_at(bulk * 4);
...
impl u32x4 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[u32]) -> (&[[u32; 4]], &[u32]) {
        let bulk = data.len() / 4;
        let (head, tail) = data.split_at(bulk * 4);
...
impl i64x2 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[i64]) -> (&[[i64; 2]], &[i64]) {
        let bulk = data.len() / 2;
        let (head, tail) = data.split_at(bulk * 2);
...
impl u64x2 {
    ...
    /// ```
    #[inline(always)]
    pub fn partition_slice(_: archmage::X64V3Token, data: &[u64]) -> (&[[u64; 2]], &[u64]) {
        let bulk = data.len() / 2;
        let (head, tail) = data.split_at(bulk * 2);

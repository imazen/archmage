...
impl f32x16 {
    ...
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[f32]) -> Self {
    ...
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[f32; 16]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = f32x16::partition_slice(token, data);
    /// for chunk in chunks {
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[f32]) -> (&[[f32; 16]], &[f32]) {
        ...
        // beyond f32 alignment, which [f32; 16] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts(head.as_ptr().cast::<[f32; 16]>(), bulk) };
        (chunks, tail)
    }
    ...
    pub fn partition_slice_mut(
        ...
        // beyond f32 alignment, which [f32; 16] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[f32; 16]>(), bulk) };
        (chunks, tail)
    }
    ...
    #[inline(always)]
    pub fn from_bytes_owned(_: archmage::X64V4Token, bytes: [u8; 64]) -> Self {
        // SAFETY: [u8; 64] and Self have identical size
        Self(unsafe { core::mem::transmute(bytes) })
    }
    ...
    pub fn log2_midp_unchecked(self) -> Self {
        ...
        unsafe {
            ...
            // y = (a - 1) / (a + 1)
            let one = _mm512_set1_ps(1.0);
            let y = _mm512_div_ps(_mm512_sub_ps(a, one), _mm512_add_ps(a, one));
    ...
    pub fn log2_midp(self) -> Self {
        unsafe {
            ...
            let is_zero = _mm512_cmp_ps_mask::<_CMP_EQ_OQ>(x, zero);
            let is_negative = _mm512_cmp_ps_mask::<_CMP_LT_OQ>(x, zero);
            let is_inf = _mm512_cmp_ps_mask::<_CMP_EQ_OQ>(x, inf);

            // Compute log2 for normal values
            let log_result = self.log2_midp_unchecked().0;

            // Apply special case results
            let result = _mm512_mask_blend_ps(is_zero, log_result, neg_inf);
            let result = _mm512_mask_blend_ps(is_negative, result, nan);
            let result = _mm512_mask_blend_ps(is_inf, result, inf);
    ...
    pub fn exp_midp_unchecked(self) -> Self {
        const LOG2_E: f32 = core::f32::consts::LOG2_E;
        unsafe { Self(_mm512_mul_ps(self.0, _mm512_set1_ps(LOG2_E))).exp2_midp_unchecked() }
    }
    ...
    pub fn cbrt_lowp(self) -> Self {
        unsafe {
            ...
            let result = _mm512_or_ps(y, sign);
            // Zero masking: cbrt(±0) = ±0 (bit hack gives garbage for zero)
            let is_zero = _mm512_cmp_ps_mask::<_CMP_EQ_OQ>(x, _mm512_setzero_ps());
            let result = _mm512_mask_blend_ps(is_zero, result, x);
    ...
    pub fn transpose_8x8(rows: &mut [Self; 8]) {
        unsafe {
            let idx_lo =
                _mm512_setr_epi32(0, 16, 1, 17, 4, 20, 5, 21, 8, 24, 9, 25, 12, 28, 13, 29);
            let idx_hi =
                _mm512_setr_epi32(2, 18, 3, 19, 6, 22, 7, 23, 10, 26, 11, 27, 14, 30, 15, 31);
            ...
            rows[0] = Self(_mm512_castsi512_ps(_mm512_permutex2var_epi32(
                s0i, idx3_lo, s4i,
            )));
            rows[1] = Self(_mm512_castsi512_ps(_mm512_permutex2var_epi32(
                s1i, idx3_lo, s5i,
            )));
            rows[2] = Self(_mm512_castsi512_ps(_mm512_permutex2var_epi32(
                s2i, idx3_lo, s6i,
            )));
            rows[3] = Self(_mm512_castsi512_ps(_mm512_permutex2var_epi32(
                s3i, idx3_lo, s7i,
            )));
            rows[4] = Self(_mm512_castsi512_ps(_mm512_permutex2var_epi32(
                s0i, idx3_hi, s4i,
            )));
            rows[5] = Self(_mm512_castsi512_ps(_mm512_permutex2var_epi32(
                s1i, idx3_hi, s5i,
            )));
            rows[6] = Self(_mm512_castsi512_ps(_mm512_permutex2var_epi32(
                s2i, idx3_hi, s6i,
            )));
            rows[7] = Self(_mm512_castsi512_ps(_mm512_permutex2var_epi32(
                s3i, idx3_hi, s7i,
            )));
        }
    }
    ...
    pub fn store_8x8(rows: &[Self; 8], block: &mut [f32; 64]) {
        unsafe {
            ...
            _mm256_storeu_ps(
                block.as_mut_ptr().add(16),
                _mm512_castps512_ps256(rows[2].0),
            );
            _mm256_storeu_ps(
                block.as_mut_ptr().add(24),
                _mm512_castps512_ps256(rows[3].0),
            );
            _mm256_storeu_ps(
                block.as_mut_ptr().add(32),
                _mm512_castps512_ps256(rows[4].0),
            );
            _mm256_storeu_ps(
                block.as_mut_ptr().add(40),
                _mm512_castps512_ps256(rows[5].0),
            );
            _mm256_storeu_ps(
                block.as_mut_ptr().add(48),
                _mm512_castps512_ps256(rows[6].0),
            );
            _mm256_storeu_ps(
                block.as_mut_ptr().add(56),
                _mm512_castps512_ps256(rows[7].0),
            );
        }
    }
...
impl f64x8 {
    ...
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[f64]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[f64; 8]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = f64x8::partition_slice(token, data);
    /// for chunk in chunks {
    ///     let v = f64x8::load(token, chunk);
    ///     // ... SIMD processing ...
    /// }
    /// for x in remainder {
    ///     // ... scalar remainder ...
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[f64]) -> (&[[f64; 8]], &[f64]) {
        ...
        // beyond f64 alignment, which [f64; 8] shares.
    ...
    /// Create from raw `__m512d` (token-gated, zero-cost).
    ///
    /// This is the safe alternative to [`from_raw`](Self::from_raw). The token
    /// proves the CPU supports the required SIMD features.
    #[inline(always)]
    pub fn from_m512d(_: archmage::X64V4Token, v: __m512d) -> Self {
        Self(v)
    }
    ...
    pub fn exp2_lowp(self) -> Self {
        ...
        unsafe {
            ...
            let xi = _mm512_roundscale_pd::<0x01>(x);
...
impl i8x64 {
    ...
    /// # Panics
    ///
    /// Panics if `slice.len() < 64`.
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[i8]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[i8; 64]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = i8x64::partition_slice(token, data);
    /// for chunk in chunks {
    ///     let v = i8x64::load(token, chunk);
    ///     // ... SIMD processing ...
    /// }
    /// for x in remainder {
    ///     // ... scalar remainder ...
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[i8]) -> (&[[i8; 64]], &[i8]) {
        ...
        // beyond i8 alignment, which [i8; 64] shares.
    ...
    /// Create from raw `__m512i` (token-gated, zero-cost).
    ///
    /// This is the safe alternative to [`from_raw`](Self::from_raw). The token
    /// proves the CPU supports the required SIMD features.
    #[inline(always)]
    ...
    pub fn shl<const N: u32>(self) -> Self {
        unsafe {
            let shifted = _mm512_slli_epi16::<N>(self.0);
            let mask = _mm512_set1_epi8((0xFFu8.wrapping_shl(N)) as i8);
            Self(_mm512_and_si512(shifted, mask))
        }
    }
    ...
    pub fn shr_arithmetic<const N: u32>(self) -> Self {
        unsafe {
            let shifted = _mm512_srli_epi16::<N>(self.0);
            let byte_mask = _mm512_set1_epi8((0xFFu8.wrapping_shr(N)) as i8);
            let logical = _mm512_and_si512(shifted, byte_mask);
            let zero = _mm512_setzero_si512();
            let neg = _mm512_cmpgt_epi8_mask(zero, self.0);
            let fill = _mm512_set1_epi8((0xFFu8.wrapping_shl(8u32.wrapping_sub(N))) as i8);
            let sign_bits = _mm512_maskz_mov_epi8(neg, fill);
            Self(_mm512_or_si512(logical, sign_bits))
        }
    }
...
impl u8x64 {
    ...
    /// Panics if `slice.len() < 64`.
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[u8]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[u8; 64]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = u8x64::partition_slice(token, data);
    /// for chunk in chunks {
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[u8]) -> (&[[u8; 64]], &[u8]) {
        ...
        // beyond u8 alignment, which [u8; 64] shares.
...
impl i16x32 {
    ...
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[i16]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[i16; 32]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = i16x32::partition_slice(token, data);
    /// for chunk in chunks {
    ///     let v = i16x32::load(token, chunk);
    ///     // ... SIMD processing ...
    /// }
    /// for x in remainder {
    ///     // ... scalar remainder ...
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[i16]) -> (&[[i16; 32]], &[i16]) {
        ...
        // beyond i16 alignment, which [i16; 32] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts(head.as_ptr().cast::<[i16; 32]>(), bulk) };
        (chunks, tail)
    }
    ...
    pub fn partition_slice_mut(
        ...
        // beyond i16 alignment, which [i16; 32] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[i16; 32]>(), bulk) };
        (chunks, tail)
    }
...
impl u16x32 {
    ...
    /// Panics if `slice.len() < 32`.
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[u16]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[u16; 32]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = u16x32::partition_slice(token, data);
    /// for chunk in chunks {
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[u16]) -> (&[[u16; 32]], &[u16]) {
        ...
        // beyond u16 alignment, which [u16; 32] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts(head.as_ptr().cast::<[u16; 32]>(), bulk) };
        (chunks, tail)
    }
    ...
    pub fn partition_slice_mut(
        ...
        // beyond u16 alignment, which [u16; 32] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[u16; 32]>(), bulk) };
        (chunks, tail)
    }
...
impl i32x16 {
    ...
    /// Panics if `slice.len() < 16`.
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[i32]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[i32; 16]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = i32x16::partition_slice(token, data);
    /// for chunk in chunks {
    ///     let v = i32x16::load(token, chunk);
    ///     // ... SIMD processing ...
    /// }
    /// for x in remainder {
    ///     // ... scalar remainder ...
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[i32]) -> (&[[i32; 16]], &[i32]) {
        ...
        // beyond i32 alignment, which [i32; 16] shares.
        let chunks =
...
impl u32x16 {
    ...
    /// Panics if `slice.len() < 16`.
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[u32]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[u32; 16]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = u32x16::partition_slice(token, data);
    /// for chunk in chunks {
    ///     let v = u32x16::load(token, chunk);
    ///     // ... SIMD processing ...
    /// }
    /// for x in remainder {
    ///     // ... scalar remainder ...
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[u32]) -> (&[[u32; 16]], &[u32]) {
        ...
        // beyond u32 alignment, which [u32; 16] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts(head.as_ptr().cast::<[u32; 16]>(), bulk) };
        (chunks, tail)
    }
    ...
    pub fn partition_slice_mut(
        ...
        // beyond u32 alignment, which [u32; 16] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[u32; 16]>(), bulk) };
        (chunks, tail)
    }
...
impl i64x8 {
    ...
    /// Panics if `slice.len() < 8`.
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[i64]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[i64; 8]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = i64x8::partition_slice(token, data);
    /// for chunk in chunks {
    ///     let v = i64x8::load(token, chunk);
    ///     // ... SIMD processing ...
    /// }
    /// for x in remainder {
    ///     // ... scalar remainder ...
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[i64]) -> (&[[i64; 8]], &[i64]) {
        ...
        // beyond i64 alignment, which [i64; 8] shares.
...
impl u64x8 {
    ...
    /// Panics if `slice.len() < 8`.
    #[inline(always)]
    pub fn from_slice(_: archmage::X64V4Token, slice: &[u64]) -> Self {
    ...
    /// Split a slice into SIMD-width chunks and a scalar remainder.
    ///
    /// Returns `(chunks, remainder)` where each chunk is a `&[u64; 8]`
    /// that can be passed directly to [`load`](Self::load).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let (chunks, remainder) = u64x8::partition_slice(token, data);
    /// for chunk in chunks {
    ///     let v = u64x8::load(token, chunk);
    ///     // ... SIMD processing ...
    /// }
    /// for x in remainder {
    ///     // ... scalar remainder ...
    ...
    pub fn partition_slice(_: archmage::X64V4Token, data: &[u64]) -> (&[[u64; 8]], &[u64]) {
        ...
        // beyond u64 alignment, which [u64; 8] shares.
        let chunks = unsafe { core::slice::from_raw_parts(head.as_ptr().cast::<[u64; 8]>(), bulk) };
        (chunks, tail)
    }
    ...
    pub fn partition_slice_mut(
        ...
        // beyond u64 alignment, which [u64; 8] shares.
        let chunks =
            unsafe { core::slice::from_raw_parts_mut(head.as_mut_ptr().cast::<[u64; 8]>(), bulk) };
        (chunks, tail)
    }

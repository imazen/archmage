//! Backend trait for `f64x4<T>` — 4-lane f64 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `f64x4<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 4-lane f64 SIMD vectors.
///
...
pub trait F64x4Backend: SimdToken + Sealed + Copy + 'static {
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as F64x4Backend>::to_array(self, lo);
        let b = <Self as F64x4Backend>::to_array(self, hi);
        <Self as F64x4Backend>::from_array(
            self,

//! Backend trait for `u16x8<T>` — 8-lane u16 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `u16x8<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 8-lane u16 SIMD vectors.
///
...
pub trait U16x8Backend: SimdToken + Sealed + Copy + 'static {
    ...
    where
        Self: super::U32x4Backend;
    /// Widen in natural lane order: result[i] = a[i + 4] as u32.
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as U16x8Backend>::to_array(self, lo);
        let b = <Self as U16x8Backend>::to_array(self, hi);
        <Self as U16x8Backend>::from_array(
            self,

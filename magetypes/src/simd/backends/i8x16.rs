//! Backend trait for `i8x16<T>` — 16-lane i8 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `i8x16<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 16-lane i8 SIMD vectors.
///
...
pub trait I8x16Backend: SimdToken + Sealed + Copy + 'static {
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as I8x16Backend>::to_array(self, lo);
        let b = <Self as I8x16Backend>::to_array(self, hi);
        <Self as I8x16Backend>::from_array(
            self,

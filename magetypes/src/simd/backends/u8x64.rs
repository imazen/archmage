//! Backend trait for `u8x64<T>` — 64-lane u8 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `u8x64<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 64-lane u8 SIMD vectors.
///
...
pub trait U8x64Backend: SimdToken + Sealed + Copy + 'static {
    ...
    ///
    /// `count >= 8` produces a sign fill on every backend.

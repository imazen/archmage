//! Backend trait for `i64x8<T>` — 8-lane i64 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `i64x8<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 8-lane i64 SIMD vectors.
///
...
pub trait I64x8Backend: SimdToken + Sealed + Copy + 'static {
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as I64x8Backend>::to_array(self, lo);
        let b = <Self as I64x8Backend>::to_array(self, hi);
        <Self as I64x8Backend>::from_array(
            self,

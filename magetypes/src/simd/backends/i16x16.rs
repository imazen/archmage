//! Backend trait for `i16x16<T>` — 16-lane i16 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `i16x16<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 16-lane i16 SIMD vectors.
///
...
pub trait I16x16Backend: SimdToken + Sealed + Copy + 'static {
    ...
    ///
    /// `count >= 16` produces a sign fill (every lane becomes
    ...
    where
        Self: super::I32x8Backend;
    /// Widen in natural lane order: result[i] = a[i + 8] as i32.
    ...
    where
        Self: super::I32x8Backend;
    /// Clamp to i8's range, then concatenate a's 16 lanes followed by b's.
    ...
    where
        Self: super::I8x32Backend;
    /// Clamp to u8's range, then concatenate a's 16 lanes followed by b's.
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as I16x16Backend>::to_array(self, lo);
        let b = <Self as I16x16Backend>::to_array(self, hi);
        <Self as I16x16Backend>::from_array(
            self,

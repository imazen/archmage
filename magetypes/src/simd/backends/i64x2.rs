//! Backend trait for `i64x2<T>` — 2-lane i64 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `i64x2<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 2-lane i64 SIMD vectors.
///
...
pub trait I64x2Backend: SimdToken + Sealed + Copy + 'static {
    ...
    /// Lanes `N..N+2` of the concatenation `[lo, hi]`.
    ///
    /// `N == 0` returns `lo`; `N == 2` would return `hi` and is rejected,
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as I64x2Backend>::to_array(self, lo);
        let b = <Self as I64x2Backend>::to_array(self, hi);
        <Self as I64x2Backend>::from_array(
            self,
            core::array::from_fn(|i| if n + i < 2 { a[n + i] } else { b[n + i - 2] }),
        )
    }
}

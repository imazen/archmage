...
pub trait U16x32Backend: SimdToken + Sealed + Copy + 'static {
    ...
    /// Widen in natural lane order: result[i] = a[i + 16] as u32.
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as U16x32Backend>::to_array(self, lo);
        let b = <Self as U16x32Backend>::to_array(self, hi);
        <Self as U16x32Backend>::from_array(
            self,

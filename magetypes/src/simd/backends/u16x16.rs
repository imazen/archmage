//! Backend trait for `u16x16<T>` — 16-lane u16 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `u16x16<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 16-lane u16 SIMD vectors.
///
...
pub trait U16x16Backend: SimdToken + Sealed + Copy + 'static {
    ...
    /// Widen in natural lane order: `result[i] = a[i + 0] as u32`.
    fn widen_low_u16_to_u32(
        self,
        a: <Self as super::U16x16Backend>::Repr,
    ) -> <Self as super::U32x8Backend>::Repr
    where
        Self: super::U32x8Backend;
    /// Widen in natural lane order: `result[i] = a[i + 8] as u32`.
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as U16x16Backend>::to_array(self, lo);
        let b = <Self as U16x16Backend>::to_array(self, hi);
        <Self as U16x16Backend>::from_array(
            self,

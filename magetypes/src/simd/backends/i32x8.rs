//! Backend trait for `i32x8<T>` — 8-lane i32 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `i32x8<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 8-lane i32 SIMD vectors.
///
...
pub trait I32x8Backend: SimdToken + Sealed + Copy + 'static {
    ...
    /// Clamp to i16's range, then concatenate a's 8 lanes followed by b's.
    ...
    /// Clamp to u16's range, then concatenate a's 8 lanes followed by b's.

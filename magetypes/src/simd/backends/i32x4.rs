...
pub trait I32x4Backend: SimdToken + Sealed + Copy + 'static {
    ...
    /// Clamp to i16's range, then concatenate a's 4 lanes followed by b's.
    ...
    /// Clamp to u16's range, then concatenate a's 4 lanes followed by b's.
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as I32x4Backend>::to_array(self, lo);
        let b = <Self as I32x4Backend>::to_array(self, hi);
        <Self as I32x4Backend>::from_array(
            self,

//! Backend trait for `i16x32<T>` — 32-lane i16 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `i16x32<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 32-lane i16 SIMD vectors.
///
...
pub trait I16x32Backend: SimdToken + Sealed + Copy + 'static {
    ...
    ///
    /// `count >= 16` produces a sign fill on every backend.
    ...
    where
        Self: super::I32x16Backend;
    /// Clamp to i8's range, then concatenate a's 32 lanes followed by b's.
    ...
    where
        Self: super::I8x64Backend;
    /// Clamp to u8's range, then concatenate a's 32 lanes followed by b's.
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as I16x32Backend>::to_array(self, lo);
        let b = <Self as I16x32Backend>::to_array(self, hi);
        <Self as I16x32Backend>::from_array(
            self,

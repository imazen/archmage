//! Backend trait for `u8x32<T>` — 32-lane u8 SIMD vector.
//!
//! Each token type implements this trait with its platform-native representation.
//! The generic wrapper `u8x32<T>` delegates all operations to these trait methods.
//!
...
/// Backend implementation for 32-lane u8 SIMD vectors.
///
...
pub trait U8x32Backend: SimdToken + Sealed + Copy + 'static {
    ...
    fn concat_shift<const N: i32>(self, lo: Self::Repr, hi: Self::Repr) -> Self::Repr {
        ...
        let n = N as usize;
        let a = <Self as U8x32Backend>::to_array(self, lo);
        let b = <Self as U8x32Backend>::to_array(self, hi);
        <Self as U8x32Backend>::from_array(
            self,

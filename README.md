...
Pipelines that store pixels as IEEE-754 binary16 (`f16`) bit patterns and decode them to `f32` for filtering can use the `F16Convert` trait in `magetypes::simd::generic` (also re-exported at `magetypes::simd`). Its converters are **methods on the archmage token** (the token is `self`, matching the magetypes token-method convention) and operate on slices:

...
**`#![forbid(unsafe_code)]` compatible.** The `unsafe` in `#[arcane]` wrappers is generated by the proc macro — your code stays fully safe. `#[rite]` helpers need no `unsafe` at all since Rust 1.85+ allows safe calls between matching `#[target_feature]` functions.

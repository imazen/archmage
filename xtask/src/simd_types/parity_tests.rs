...
pub fn generate_parity_tests() -> Vec<(String, String)> {
    // `polyfill_parity.rs` is retired: it compared the legacy concrete
    // `polyfill::v3` types against the concrete `x86::w256` natives, both of
    // which no longer exist. Cross-architecture consistency of the generic
    // types is covered by `cross_arch_parity.rs`.
    vec![
        (

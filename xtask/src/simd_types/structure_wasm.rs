...
fn generate_construction_methods(ty: &SimdType) -> String {
    ...
    formatdoc! {r#"
        ...
        #[inline(always)]
        pub fn partition_slice(_: archmage::Wasm128Token, data: &[{elem}]) -> (&[[{elem}; {lanes}]], &[{elem}]) {{
        ...
        #[inline(always)]
        pub fn partition_slice_mut(_: archmage::Wasm128Token, data: &mut [{elem}]) -> (&mut [[{elem}; {lanes}]], &mut [{elem}]) {{
            let bulk = data.len() / {lanes};

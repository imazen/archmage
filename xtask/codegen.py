...
def integer_probes():
    ...
    for target, (arch, tiers) in targets.items():
        ...
        for name in names:
            ...
            if not equal and not pair_equivalent and not name.endswith("sad_composed"):
                failures.append((target, name, api, hand))
            rows.append({"name": name, "identical": equal, "pair_dataflow_equal": pair_equivalent, "api": api, "hand": hand})
        result[target] = rows
        print(
            target,

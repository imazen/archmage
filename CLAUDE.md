...
- **magetypes: `concrete-types` feature to gate the generated concrete platform types.** The concrete types (`simd::v3::f32x8`, `simd::neon::f32x4`, etc. in `generated/`) are 53K lines / 5,193 pub methods contributing ~5MB rmeta. Most users only need the generic backend API (`generic::f32x8<T>` via `backends/` + `impls/`). Adding a `concrete-types` feature (default: on) would let users who only use the generic pattern opt out and save ~5MB / ~37% of magetypes' 10.9MB rmeta. All four layers (generated/, backends/, impls/, generic/) are xtask-generated. The concrete types have no dependency on the generic layer, but the generic layer wraps them — so `concrete-types` must be on when `generic` is on (or just always include the platform types the generic layer needs). Only linear-srgb uses concrete types directly (v4::f32x16 for AVX-512).

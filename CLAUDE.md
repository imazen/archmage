...
5. **magetypes `f32→i32` conversion diverges across backends on out-of-range/NaN lanes** — OPEN, [#80](https://github.com/imazen/archmage/issues/80). x86 `_mm*_cvttps_epi32` yields the `i32::MIN` integer-indefinite sentinel for OOR and NaN; scalar (`as`), NEON (FCVTZS), and wasm (`trunc_sat`) saturate with NaN→0 — so `3e9_f32` converts to −2147483648 on x86 and +2147483647 everywhere else, silently. Suggested shape: document the native op's per-backend rails + add a saturating variant contracted uniformly (compare/blend fixup on x86). Audit `_round`, f64, and u32 variants at the same time. Found via the fearless_simd review (their #134/#167/#348 fixed the same class).

6. ~~**magetypes `recip()`/`rsqrt()` returned NaN at `±0`/`±inf` on estimate-refining backends**~~ — Fixed across three rounds: NEON f32 (defbbc2/1b36fc7), NEON f64 + generator idempotence (the 0.9.28+ remote work), and finally x86 all widths incl. 512-bit V4/V4x (dec6a46, [#64](https://github.com/imazen/archmage/issues/64) closed). Newton refinement of a hardware estimate computes `a*r = inf*0 = NaN` at the rails and misses the 0 ULP "full precision" contract; every backend now divides. The x86 flip was deliberately deferred until measured on x86 silicon — `benchmarks/recip_x86_zen5-9950x3d_2026-09-03.md` (Zen 5: exact ~1.9x/~3.6x slower in an L1 microbench; shipped anyway, correctness wins in the full-precision tier; `rcp_approx`/`rsqrt_approx` remain the throughput tier and still NaN at rails on ARM's fused-refine path, documented). Guarded by `precise_reciprocals.rs` (0 ULP + rails, every backend/width) and `sigmoid_silu.rs` (the #64 repro shape; `sigmoid_midp`/`silu_midp` added in 817cdd6).
...
| `to_i32` on OOR/NaN lanes | `i32::MIN` sentinel (cvttps) | Saturates, NaN→0 (FCVTZS) | Saturates, NaN→0 (trunc_sat; scalar `as` likewise) | OPEN divergence, #80 — keep lanes in range until a saturating variant lands |

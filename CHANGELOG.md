...
- magetypes: **capability-probe build script** (`magetypes/build.rs`) that try-compiles newer-stable `core::arch` intrinsics for the build target and emits a `cargo:rustc-cfg` (with matching `cargo:rustc-check-cfg`) only when they compile. This lets a hardware path that uses an intrinsic stabilized *above* the crate MSRV light up automatically on a capable toolchain **without raising the MSRV** (still 1.89): on an older toolchain the probe fails, the cfg stays unset, and the already-shipped branchless software kernel is used. The probe is feature-detection (autocfg-style try-compile), not version-number matching, so it is robust to target variance and backports; it shells out to the same `$RUSTC` Cargo invoked it with, adding **no build-dependency**. The pattern generalizes — adding the next newer-stable intrinsic (AVX-512-FP16, SVE, …) is a one-snippet addition (see the `build.rs` module docs).
- magetypes: **native aarch64 NEON-f16 hardware backend** for the f16 slice converters, un-deferred via the capability gate above. When compiled with Rust ≥ 1.94 (probe sets `archmage_has_neon_f16`) **and** the CPU presents `fp16` at runtime (the `Arm64V2Token` tier: NEON+FP16, i.e. Cortex-A55+/Apple M1+/Graviton 2+), a `NeonToken` routes the slice converters through native `vcvt_f32_f16` / `vcvt_f16_f32` (4 lanes per op); otherwise it keeps the branchless software kernel — same runtime-token dispatch the F16C path uses. The intrinsics are encapsulated behind `#[archmage::arcane]` (the `Arm64V2Token` tier enables `fp16` in the emitted `#[target_feature]` region) and memory ops use the safe `safe_unaligned_simd` re-exports, so no module-level `unsafe`. Verified **bit-identical to the software path** under QEMU (`-cpu max`): all 65 536 f16 (decode) and the boundary-band + dense-sweep encode coverage, with the same benign **NaN-only** divergences as F16C — decode hardware-quiets exactly the 1022 f16 *signaling* NaNs, encode may emit a different (valid) f16 NaN payload. On a toolchain < 1.94 the same source compiles cleanly with the software path and **no MSRV bump** (proven by `cargo +1.93 check --target aarch64-unknown-linux-gnu`). Public API unchanged; `cargo semver-checks` reports no breaking change.

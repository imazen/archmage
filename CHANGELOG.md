...
magetypes. It is purely additive — new converter functions and hardware
...
- magetypes: branchless, vectorized **f16 (IEEE-754 binary16) ↔ f32** converters in `simd::generic::convert_f16`, generic over the backend token, so NEON / WASM128 / SSE / scalar all get a straight-line SIMD path instead of an element-by-element software decode. Re-exported at `simd::{f16_to_f32x4, f32_to_f16x4, f16_to_f32_slice, f32_to_f16_slice}` and `simd::generic::*`. Decode uses Fabian Giesen's magic-multiply (`2^112` rescale, branchless Inf/NaN fixup); encode uses Giesen's `float_to_half_full_rtne` (RTNE magic-add, subnormal-flush, ±Inf saturation, NaN→canonical qNaN). Verified **bit-identical to a scalar IEEE reference exhaustively**: all 65 536 f16 decode inputs (incl. NaN bit patterns) and **all 2³² f32 encode inputs** (finite/Inf; NaN→NaN per the documented payload-may-differ contract) — `tests/convert_f16_exhaustive.rs`. The in-register kernels are pure safe arithmetic (no `unsafe`, no intrinsics), so correctness is arch-independent (b3862b9).
- magetypes: **native x86-64 F16C hardware backend** for the f16 slice converters. The slice entry points now dispatch through a new sealed `F16Convert` trait (re-exported at `simd::F16Convert` / `simd::generic::F16Convert`): an `X64V3Token` (x86-64-v3, whose tier includes `f16c`) routes through the native `vcvtph2ps` / `vcvtps2ph` instructions, processing 8 lanes per AVX op; every other token keeps the branchless software kernel. AVX-512 (`X64V4Token`) holders reach the F16C path via `token.v3()`. The intrinsics are encapsulated behind `#[archmage::arcane]` boundaries (the framework's blessed `#[target_feature]` trampoline), so the module stays free of module-level `unsafe` and the public functions remain safe to call from ordinary code; memory ops use the safe `safe_unaligned_simd` re-exports (no raw pointers). The hardware path is verified **bit-identical to the software path** over all 65 536 f16 (decode) and the boundary-band + dense-sweep encode coverage, with two documented benign **NaN-only** divergences: decode hardware-quiets the 1022 f16 *signaling* NaNs (`vcvtph2ps` sets the f32 mantissa MSB; the software path widens the signaling payload — both valid f32 NaNs), and encode may emit a different (still valid) f16 NaN payload. F16C is stable since Rust 1.68. The public API and signatures are unchanged at the call site; the bounds tightened from `T: F32x4Convert` to `T: F16Convert` (sealed, blanket-implemented for every token that had `F32x4Convert`), so `cargo semver-checks` reports **no breaking change** (additive / semver-minor) (e25aa84).
- magetypes: **toolchain-version gate** (`rustversion` + `target_arch`) for hardware paths that use a `core::arch` intrinsic stabilized *above* the crate MSRV — replacing the trialed build-script capability-probe for the stable case. A stable intrinsic has a nameable stabilization version, so the path is selected by `#[rustversion::since(X)]` / `#[rustversion::before(X)]` inside its `#[cfg(target_arch = "…")]` scope: on rustc ≥ X the hardware kernel compiles and a runtime token decides whether to use it; on rustc < X only the branchless software fallback compiles — **MSRV unchanged (1.89), no missing-intrinsic error**. `rustversion` 1.0.22 is a dep-light, trusted proc-macro (no build script, no bespoke-probe gotchas like a cross-`core` false-negative). The named version bound is made load-bearing by a new CI matrix (see below) that builds + tests **both sides** of the boundary. The dropped build-script probe was measured (~80 ms one-time cold compile, ~0 hot — `benchmarks/build_script_overhead_*.md`); `rustversion` + CI matrix was chosen over it for maintainability. A slim `magetypes/build.rs` is retained **only** as a nightly-opportunistic try-compile probe scaffold (see below) — it does nothing on stable/beta (c6efa9f, 40bd485).
- magetypes: **native aarch64 NEON-f16 hardware backend** for the f16 slice converters, un-deferred via the version gate above. When compiled with Rust ≥ 1.94 (the `#[rustversion::since(1.94)]`-gated kernels compile) **and** the CPU presents `fp16` at runtime (the `Arm64V2Token` tier: NEON+FP16, i.e. Cortex-A55+/Apple M1+/Graviton 2+), a `NeonToken` routes the slice converters through native `vcvt_f32_f16` / `vcvt_f16_f32` (4 lanes per op); otherwise it keeps the branchless software kernel — same runtime-token dispatch the F16C path uses. The intrinsics are encapsulated behind `#[archmage::arcane]` (the `Arm64V2Token` tier enables `fp16` in the emitted `#[target_feature]` region) and memory ops use the safe `safe_unaligned_simd` re-exports, so no module-level `unsafe`. Verified **bit-identical to the software path** under QEMU (`-cpu max`): all 65 536 f16 (decode) and the boundary-band + dense-sweep encode coverage, with the same benign **NaN-only** divergences as F16C — decode hardware-quiets exactly the 1022 f16 *signaling* NaNs, encode may emit a different (valid) f16 NaN payload. On a toolchain < 1.94 the same source compiles cleanly with the software path and **no MSRV bump** (proven by `cargo +1.93 check --target aarch64-unknown-linux-gnu`). Public API unchanged; `cargo semver-checks` reports no breaking change (c6efa9f, 40bd485).
- magetypes: **CI matrix `f16-version-gate`** that builds + runs the exhaustive `convert_f16` bit-identity test on `aarch64-unknown-linux-gnu` (via `cross`, QEMU `QEMU_CPU=max` so `fp16` is present) across **rustc 1.93 / 1.94 / stable / nightly** (`fail-fast: false`). This is the safety net that makes the `#[rustversion::since(1.94)]` version bound load-bearing: the 1.93 cell proves the pre-stabilization software fallback compiles with no MSRV violation and tests pass (software-vs-software, 0 NaN divergence); the 1.94 cell proves the gate flips on and the intrinsic resolves; stable/nightly give ongoing coverage. The test reads a `rustversion`-gated `neon_f16_hw_compiled()` helper so it expects the HW path's 1022-NaN divergence **only** when the kernel is actually compiled (≥ 1.94), not merely when the CPU reports `fp16`. Existing CI jobs (native ARM `test-aarch64`, `test-cross`, `msrv`, windows-11-arm/macos-intel/i686 coverage) are unchanged (40bd485).
- magetypes: **nightly-opportunistic try-compile probe scaffold** (`magetypes/build.rs`, gated nightly-only). For an intrinsic that is *nightly-only* (no stable version yet, so `rustversion` can't name a bound and a blind `#![feature(…)]` breaks across nightlies), the robust gate is a try-compile probe run only under nightly that enables the path iff it compiles on *this* nightly. archmage has no such intrinsic today, so this ships as a documented scaffold: on nightly the build script try-compiles a trivially-stable placeholder snippet and sets `archmage_nightly_probe_example` (wired into a `#[rustversion::nightly] #[cfg(…)]` const so the pattern compiles end-to-end and the CI nightly cell exercises it); on stable/beta it does nothing. The decision (stable intrinsics → `rustversion` + CI both-sides; nightly-only → probe) is documented in `MSRV.md` (c6efa9f, 40bd485).

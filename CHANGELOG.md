...
- magetypes: **the fast per-platform `rcp_approx` / `rsqrt_approx` (f32) now guarantee a ≥~12-bit floor** by the cheapest *measured* path on each target, instead of forwarding a bare hardware estimate everywhere. x86 keeps the raw `rcpps`/`rsqrtps` estimate (~12-bit); **ARM adds one Newton step** to the ~8-bit `vrecpe`/`vrsqrte` estimate (~16-bit). On **WASM/scalar** (no hardware estimate): `rcp_approx` is exact division (~24-bit — benchmarked as fast as, and more accurate than, a bit-hack), while **`rsqrt_approx` is an integer bit-hack seed + 2 Newton steps (~17-bit)** replacing the expensive `sqrt`+division — **measured 1.9× faster on WASM and 11× faster on scalar** (`benchmarks/wasm_scalar_reciprocal_2026-06-21.md`), and bit-identical to `rsqrt_approx_portable` + one `rsqrt_newton_portable`. Previously ARM/WASM callers got the unrefined ~8-bit estimate (or identity), and WASM/scalar `rsqrt_approx` paid for a full `sqrt`+division. Dispatch keys off two new backend consts (`HW_RECIP_ESTIMATE` / `HW_RECIP_ESTIMATE_12BIT`) and adds **no** trait bounds to the generic API. Measured floors (`magetypes/tests/reciprocal_precision.rs`): x86 11.9/12.2-bit, ARM 17.1/16.5-bit, WASM/scalar 24.3/17.7-bit. For the *same bits on every machine* use the `_portable` family above; for full precision use `recip`/`rsqrt`.

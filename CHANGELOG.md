...
- Windows-on-ARM detection now routes unconditionally through `winarm-cpufeatures` ≥ 0.1.1's `is_aarch64_feature_detected_full!` macro (registry-backed `ID_AA64*_EL1` decoder + `IsProcessorFeaturePresent`). Every aarch64 token's `summon()` slow path inherits the wider coverage automatically — recovers ~30 feature names that stdarch's IPFP-only Windows backend cannot see. Sandboxed callers can disable the registry layer at runtime via `winarm_cpufeatures::set_registry_enabled(false)`. The dep is target-scoped to `cfg(all(target_os = "windows", target_arch = "aarch64"))` and the crate is internally cfg-gated to that combo, so it never resolves on any other target. No cargo feature flag required. (Initial wiring used `_fast`, which is IPFP-only and silently dropped registry-classified names like `fhm`/`fcma`/`sha3`/`i8mm`/`bf16` — the Cobalt 100 hardware assertion below caught it before any release.)

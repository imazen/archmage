...
- magetypes: **ARM/NEON `rcp` / `rsqrt` now use the native Newton-Raphson assist instructions** FRECPS (`vrecpsq`) and FRSQRTS (`vrsqrtsq`) instead of hand-rolled `mul`/`sub`, and the `_approx` fast path applies one fused refinement step rather than returning the raw hardware seed. This tightens the cross-architecture precision spread that `rsqrt_approx` / `rcp_approx` carried: the raw NEON seed is only ~8-bit, versus x86's ~12-bit hardware estimate and WASM's full-precision division. **Measured (QEMU aarch64, 120k-input sweep, `magetypes/tests/rsqrt_precision.rs`):** `rsqrt_approx` ~8.3-bit → ~15.9-bit, `rcp_approx` ~8.5-bit → ~16.9-bit; full `rsqrt` / `recip` stay at ~23-bit (unchanged 1e-5 parity contract). x86 (~12-bit approx, ~22-bit full) and WASM (full) are unchanged. The full paths also drop the 2.0 / 3.0 / 0.5 constant splats, and all three ARM code paths (native f32x4, polyfill f32x8/f32x16, legacy concrete struct) are now consistent — the legacy struct previously did only one Newton step for "precise" `rsqrt` (~16-bit). f64 rides the same code (~16-bit approx, ~32-bit full). `scalar_parity`'s `_approx` tolerance is tightened 4e-3 → 1e-3 to pin the new floor (2ad2f17).

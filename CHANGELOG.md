...
- **`Token::from_context()`** constructs a token without detection inside a matching native target-feature context. Generated for every real token using its full tier feature list; safe native calls require a matching or stronger caller context. Scalar is always safe; WASM follows Rust's platform-specific safe-call rules. Foreign-architecture stubs omit the method.

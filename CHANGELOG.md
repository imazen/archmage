...
- **magetypes: `concat_shift` — cross-vector element shift ("concatenate two vectors, take a window"), on the existing backend traits.** `lo.concat_shift::<N>(hi)` yields lanes `N..N+LANES` of `[lo, hi]`; Intel calls it `valignd`/`palignr`, ARM calls it `EXT`, wasm spells it `i8x16.shuffle`. A 3x3 convolution needs three W-lane vectors at `x-1, x, x+1`, and loading all three costs three loads where two adjacent loads plus two shifts suffice. Measured in isolation on a 7950X with 96 taps and 8 accumulator chains: three unaligned loads 125.1 GFLOP/s, two aligned loads + two `valignd` 142.6, **two UNALIGNED loads + two `valignd` 141.9** — +13%, and **alignment is irrelevant**, so what the load path saves is load COUNT rather than cache-line splits.
  - **Read that +13% as an upper bound on one effect in isolation, not a prediction.** In zensr's real conv3x3 the same substitution is +0.8% (3/5) on AVX-512 — noise — and the tier that gains is AVX2, which the isolated harness never measured. Two loads a full vector apart also touch MORE cache lines than three overlapping ones, so at large sizes the substitution raises the L1 miss rate by half and has to be paid for elsewhere (zensr paid with a one-cache-line row-stride skew). Details: `zensr/benchmarks/conv3x3_tap_load_2026-09-08.md`.
  - **A provided method on ALL 30 backend traits, plus a `concat_shift` wrapper on all 30 generic types.** No new trait: code already generic over the backend gets it with no extra bound to thread. Generated by `xtask/src/simd_types/concat_shift_gen.rs`.
  - **Every element type, not just f32.** Every ISA spells this byte-granular or per-element for all of them — `palignr` and `i8x16.shuffle` do not care what the lanes mean, and NEON has a `vextq_` for each — so an f32-only operation would have been an arbitrary line, not a hardware one. An integer funnel shift is what a byte-shuffling kernel reaches for: deblocking filters, LZ match copy, pixel-format conversion and CRC folding all hand-roll it per-arch today.
  - **Native on every tier for every type** (the intrinsic each override calls; see below for what LLVM actually emits): AVX2 `vperm2x128`+`vpalignr` and SSSE3 `palignr` at 256 and 128 bits on `X64V3Token` — 30 of 30; `vextq_*` on NEON and `i8x16.shuffle` on wasm at 128 bits, with the 256- and 512-bit polyfills delegating per sub-vector to that leaf — 30 of 30 each; AVX-512 `valignd`/`valignq` for 32- and 64-bit elements. `ScalarToken` keeps the default, which is optimal there because its `Repr` *is* the array.
  - **8- and 16-bit elements at 512 bits have no full-width byte funnel shift** — `vpalignr` is per-128-bit-lane at every width — so they are built from the dword shift, which is full-width. For a byte offset `s`, take `q = s / 16` whole 128-bit lanes and `r = s % 16` bytes: `valignd` reaches the windows at `16q` and `16q + 16` (both dword-aligned), and the second one's lane `i` begins exactly 16 bytes after the first's, which is what makes a per-lane `vpalignr` by `r` land on `[s + 16i, s + 16i + 16)`. At `q == 3` the upper window is `hi` itself, which is also the only case where the immediate would leave range. Three instructions at worst, one at best — and `scripts/verify-asm.sh` now gates the integer paths too, so the claim is checked rather than asserted: `u8x16` at shift 3 emits **one `vpalignr`**, and `i16x32` at shift 9 (a shift that crosses a 128-bit lane) emits **`vshufi64x2` + `valignq` + `vpalignr`** — the decomposition, with LLVM substituting qword-granular equivalents for the two dword shifts.
  - **The default body is the fallback, not a fast path.** MEASURED 2026-09-08 by disassembling it inlined into a `target_feature` region: the lane gather is 6 ops (f32x8, AVX2, N=1) and 7 ops (f32x16, AVX-512, N=1) where the native forms are 2 and 1. LLVM does not recover the funnel shift, which is why every ISA that has the instruction overrides.
  - **The mnemonic is LLVM's choice, and it moves.** The same AVX-512 shift has been observed as `valignd` (fat LTO), `vpermi2ps` (bench build) and `vpermt2ps` with a loop-hoisted mask register (zensr's real kernel) — all one instruction per shift; the AVX2 one as `vperm2i128`+`vpalignr`, and as `vperm2f128`+`vshufps`/`vshufpd` with the permute CSE'd across both shifts. `scripts/verify-asm.sh` therefore gates the **absence of per-lane scalar moves** rather than a mnemonic: selection among the good encodings is not a contract, falling back to the gather is.
  - `N` is `i32` because that is the type of the ISA immediates it forwards to — a const generic cannot be cast in a const position, so `_mm512_alignr_epi32::<N>` only works if `N` already has the immediate's type.
  - `magetypes/tests/concat_shift.rs` checks every summonable token on every type against the lane-gather reference. Shifts are exhaustive up to 16 lanes; on the 32- and 64-lane types they are sampled either side of every 128-bit lane crossing, which is where a per-lane instruction stops matching a full-width shift. Values are `i` and `lanes + i`, which stay distinct per lane and inside `i8::MAX` at every width, so one scheme covers f32 through u8.
...
- **`Token::from_context()` — a safe `#[target_feature]` constructor that makes rustc the arbiter of every token construction ([#87](https://github.com/imazen/archmage/pull/87)).** Inside a `#[target_feature]` region — an `#[arcane]`, `#[rite]` or `#[magetypes]` body — the attribute already *is* the proof, so the call needs no `unsafe`: `let token = X64V3Token::from_context();` in a `#[rite(v3)]` body compiles under `#![forbid(unsafe_code)]`. Generated for all 17 tiers from each one's complete feature list. `forge_token_dangerously()` becomes a deprecated alias with the identical gate, so existing `unsafe { forge_token_dangerously() }` call sites still compile.
  ...
  - **Prefer it to `summon().unwrap()` inside a proven region,** which is a latent panic rather than just a wasted check: `testable_dispatch` and `dangerously_disable_token_process_wide()` make `summon()` return `None` on purpose so dispatch can be exercised down the lower tiers, and the `unwrap()` then fires in code that provably has the features and is already executing them. Measured: construction keeps working across a disable, `summon().unwrap()` panics.
  - **The feature gate is free.** `#[inline(always)]` is not permitted on a `#[target_feature]` fn, so this is plain `#[inline]` and LLVM will not inline it into the feature-free functions that call it (`summon()`, the cold detect fns, the extraction methods, `IntoConcreteToken`). It does not need to — the body constructs a ZST and emits no instructions, so the call is dead-code-eliminated instead. Verified with `cargo asm` across five probes, generic and `-C target-cpu=x86-64-v3`: no constructor call survives in any probe, `call` counts are unchanged, every probe is instruction-identical under `-C target-cpu=x86-64-v3` (bare `summon()` is still `mov eax, 1; ret`), and the generic builds move by −2/+0/+1/+0/−1 instructions — branch layout, not work.
  - rustc rejects constructing a stronger token from a weaker context, constructing from no context without `unsafe`, and coercion to a safe function pointer. Features enabled only *globally* (`-C target-feature`, `-C target-cpu=native`) do not count — rustc requires them on the caller's own attribute. Pinned by `tests/from_context.rs` (`#![forbid(unsafe_code)]`) and four `tests/soundness/from_context_*.rs` (driven by `tests/soundness_exploits.rs`) cases.
  - Foreign-architecture stubs get no `from_context()` at all — no `#[target_feature]` context for those features can exist there, so there is nothing for rustc to check; only the deprecated `unsafe fn` alias remains. `ScalarToken::from_context()` is an ungated safe `const fn`, since it asserts no features. The `forge-token-api` feature becomes a backwards-compat no-op: the constructors are always public. That gate had been excluding `tests/token_downcast.rs` from CI entirely; it now runs.

- **magetypes: `impl_pod!` now requires a field-byte total, closing the one `Pod` violation with no backstop.** Padding is invisible to every existing check — a padded type has an ordinary size and alignment, so the call-site asserts all pass, and then `copy`/`cast` reach `transmute_copy`, which reads `size_of::<Dst>()` bytes including the uninitialized padding (Miri: `constructing invalid value: encountered uninitialized memory`). Each registration now declares the sum of its type's field sizes, asserted against `size_of`; for a padded type the honest sum is strictly smaller, so it fails to compile. Eagerly evaluated as a `const _`, not post-monomorphization.

- **magetypes: generated coverage for every backend method with a bare `simd_storage` call.** The helpers' size/alignment asserts are inline `const` blocks in generic functions, so they are evaluated at codegen rather than type-check — a call site nothing instantiates is never checked. Measured with `-Zprint-mono-items` across all 40 test targets: **70 of the 180 host-instantiable backend call sites were never instantiated** (26 `load`, 26 `store`, 18 `reduce_add`), and corrupting `U8x32Backend::load` to read 64 bytes from a 32-byte array compiled cleanly through the whole suite. Not unsound — an uninstantiated function is never emitted, so the failure mode is a compile error deferred to the first caller — but CI was not checking them. `generate_simd_tests` now emits a `load`/`store`/`from_array`/`to_array`/`reduce_add` round-trip for all 30 types on every native token: **0 of 180 unverified**. The 146 ARM/WASM sites are `cfg`'d out on an x86 host and remain unmeasured.

- **Soundness: `#[arcane]`/`#[rite]` now authenticate a tier-trait bound, closing [#99](https://github.com/imazen/archmage/issues/99).** The macros select a tier's `#[target_feature]` list from the trait *name* in the signature, so a downstream crate could declare its own `pub trait HasX64V2 {}`, use it as the token bound, and receive the whole feature set with no token, no sealing and no runtime detection — in ordinary safe code, with no diagnostic. On a CPU without the features that is SIGILL. Verified reachable through `impl Trait`, a generic parameter, a where-clause, and `#[rite]`; concrete tokens were already covered by their tier-tag const.

  The emitted code now re-states each tier trait through an absolute `::archmage::` path, which a local trait cannot shadow, and requires the token value to satisfy it:
  ...
  The tier traits are sealed through `SimdToken`, so only a genuine token of that tier or stronger passes; rustc supplies the diagnostic itself ("implements similarly named trait `HasX64V2`, but not `archmage::HasX64V2`"). A real but *weaker* token is rejected too, closing the trait analogue of the token-aliasing hole. No trait definition changed, so the tier traits stay dyn-compatible and the fix carries no semver break; a `const fn` so it provably has no runtime body, taken by reference so the form works for `impl Trait` in argument position. `#[magetypes]` and `#[autoversion]` inherit it by delegation. Pinned by four cases in `tests/soundness_exploits.rs`; the `token_trait_bound` and `rite/trait_bound` expansion snapshots show the emitted assertion.

- **`cargo xtask validate` now rejects non-portable compile-fail snapshots.** trybuild compares rustc's rendered stderr byte-for-byte, so a case whose diagnostic varies by host or toolchain passes only on the machine that blessed it. The check rejects a committed `.stderr` that names target features, quotes the build configuration's enabled set, embeds an absolute or toolchain path, carries a rustc version (clippy help URLs do), or depends on pointer width, and points the author at `tests/soundness_exploits.rs` — which asserts an error code plus message fragments drawn from our own identifiers rather than rustc prose. Found the hard way: the `from_context` rejection cases were blessed on Linux x86-64, whose note reads "the sse and sse2 target features being enabled in the build configuration", while macOS-Intel renders "the cmpxchg16b, sse, sse2, sse3, sse4.1, and ssse3" for the same code.

- **Soundness scanner: two new structural rules covering the `simd_storage` abstraction.** `unsafe impl Pod` outside `magetypes/src/simd_storage.rs` is rejected (Pod asserts every bit pattern is valid, which would let the byte-cast helpers fabricate a token-bearing wrapper out of arbitrary bytes), and every `unsafe impl TokenStorage` type must be declared `#[repr(C)]` — the trait's contract is "repr(C) pair of a Pod representation and a 1-ZST token", of which only the token half was mechanically checked. Both verified to fire against injected violations.

- **magetypes: width-polyfill `all_true`/`any_true` combine sub-vectors with a vector AND/OR before one reduction** instead of reducing each half and short-circuiting in scalar — fearless_simd's polyfill lesson (via #81), measured +10.2% on a realistic early-false predicate loop over `i8x64` on the v3 `[__m256i; 2]` path (4740 vs 5280 ns, interleaved min-of-400). Applies to the 2x and 4x polyfills of every W512 integer and float type; semantics unchanged (no lane has side effects to short-circuit past), pinned by the existing boolean-reduction suites on all backends.
- **magetypes: `to_i32_saturating()` on the f32 types — uniform f32→i32 conversion, closes [#80](https://github.com/imazen/archmage/issues/80).** Bare `to_i32()` keeps each backend's native single instruction and now documents its divergence (x86 `cvttps` yields the `i32::MIN` sentinel for overflow in both directions AND NaN; NEON/WASM/scalar saturate with NaN→0 — `3e9_f32` was −2147483648 on x86 and +2147483647 everywhere else, silently). The new variant is contracted as Rust scalar `as` semantics on every backend: saturate to `i32::MIN`/`i32::MAX`, NaN→0. Native on NEON/WASM/scalar (their ops already conform); on x86 a 4-op `cvttps` + compare/blend fixup (mask-register form on the native 512-bit V4/V4x impls). Pinned lane-for-lane against `as` on every backend and width by `magetypes/tests/convert_saturating.rs`, which also pins the documented bare-op x86 sentinel so the divergence can't drift. Found via the fearless_simd review (#81; their #134/#167/#348 fixed the same class).

- **magetypes: `recip()`/`rsqrt()` are the working tier — ≤4 ULP AND exact IEEE rails, branchlessly — and `recip_portable`/`rsqrt_portable` are the precise tier; zero new names.** The unreleased exact-everywhere flip fixed #64's NaN rails but cost ~1.9x/~3.6x on Zen-class x86 and silently regressed real zen callers (iwssim uses `.recip()` as a speed idiom). The bare names now keep the fast estimate+Newton lowerings AND contract the rails, via three probe-verified branchless techniques: an FMA-Newton + `cmp_unord`+`blendv` rescue on x86 128/256-bit (the error term is NaN exactly on rail lanes; the raw estimate it rescues to is rail-exact — measured +2.6% recip / +20% rsqrt vs plain Newton, against +21%/+232% for exact division); a one-instruction `VFIXUPIMM` patch on the native AVX-512 512-bit impls (tables silicon-verified; +11.7%); and on NEON, hardware that was designed for this — FRECPS/FRSQRTS special-case `inf·0`, so `recip` was never rail-broken (earlier docs corrected) and `rsqrt` is fixed for free by forming the product inside the fused op (`y·FRSQRTS(a, y·y)`). WASM/scalar and all f64 divide (0 ULP). Subnormal inputs are the one case left unspecified at the working tier — `_portable` covers them (0 ULP, bit-identical). `sigmoid_midp`/`silu_midp` keep exact division internally so saturated lanes stay exactly 0/1. Pins on every backend and width: `precise_reciprocals.rs` (≤4 ULP + rails on bare, 0 ULP + rails on `_portable`).

- **magetypes: `sigmoid_midp()` / `silu_midp()` on `f32x4`/`f32x8`/`f32x16`** ([#64](https://github.com/imazen/archmage/issues/64)): rail-safe NN activations in the generic transcendental layer, built on the now-exact `recip()` — pre-activations at ±100 give clean 0/1 (sigmoid) and −0.0/x (silu) lanes instead of NaN. Guarded by `magetypes/tests/sigmoid_silu.rs` on scalar/x86/NEON/WASM. (817cdd6)
- `recip_x86` bench + `benchmarks/recip_x86_zen5-9950x3d_2026-09-03.md`: the x86 half of the exact-vs-Newton reciprocal measurement the NEON fix deferred. (dec6a46)

- **magetypes: integer widening and saturating narrowing.** `widen_low()` / `widen_high()` on `u8xN`/`i8xN`/`u16xN`/`i16xN` (to `u16`/`i16`/`u32`/`i32`), and `narrow_saturating_i8`/`_u8` on `i16xN` plus `narrow_saturating_i16`/`_u16` on `i32xN`, at all three widths. Widening is one native instruction per half at every tier, in natural lane order. Narrowing is offered **only** in the signed-source shape, because x86 and wasm have no unsigned-source narrowing instruction at all — `packus` clamps its input as `i16`, so a generic `u16 -> u8` saturating narrow would return `0` on x86/wasm and `255` on NEON for every lane above `0x7FFF`; typing the op on `i16xN`/`i32xN` makes that divergence unrepresentable. Truncating narrowing and the rounding narrowing shift stay unexposed (NEON-only as single instructions). Two lane-order traps are handled in the backends rather than by callers: AVX2's `_mm256_pack*` emits `[a.lo, b.lo, a.hi, b.hi]` in 64-bit groups and gets a `_mm256_permute4x64_epi64::<0xD8>` fixup, and `_mm512_pack*` (which scrambles across all four 128-bit lanes) is avoided entirely in favour of the natural-order `VPMOVSWB`/`VPMOVSDW` family plus `_mm512_inserti64x4`. Audit: `docs/CROSS-ISA-INT-PRIMITIVES.md`; measured cost: `benchmarks/int_widen_narrow_apple-m4-pro_2026-09-03.md` (NEON: 2.5-2.9x faster than the array round trip in the compute-shaped `u8 -> u16 -> u32` chain at 16-256 elements, parity at the bandwidth ceiling; a reproducible 1.5-2.2x **regression** in the *scalar* fallback's copy-shaped widening is reported there rather than hidden).

- **magetypes: uniform (runtime-count) shifts on 16/32-bit integers and saturating add/sub on 8/16-bit integers.** `shl_uniform(count)`, `shr_logical_uniform(count)`, `shr_arithmetic_uniform(count)` (signed only) and `saturating_add`/`saturating_sub` at all three widths. Runtime byte shifts are deferred until a concrete consumer needs them; constant byte shifts remain available. The shift count is a runtime `u32` applied identically to every lane — the form a CDEF-style `max(damping - msb(strength), 0)` needs, which otherwise forces one monomorph per reachable shift amount. Out-of-range counts are contracted rather than left to the ISA: `count >= lane_bits` yields zero for `shl`/`shr_logical` and a sign fill for `shr_arithmetic`, on every backend. Saturating add/sub deliberately stops at 16 bits and per-lane variable shift is deliberately absent — x86 has no `_mm*_adds_epi32` at any tier, wasm128 has no 32-bit `*_add_sat`, per-lane 16-bit shift needs AVX-512BW+VL, and wasm has no per-lane variable shift at any width. The full per-tier availability and semantics audit is `docs/CROSS-ISA-INT-PRIMITIVES.md`; measured cost is `benchmarks/int_uniform_shift_apple-m4-pro_2026-09-03.md` (NEON: +13-15 % at 16 lanes, +1.8 % at 4096, nothing measurable from 65536 up, against the shift-by-constant it replaces).
...
- **magetypes: x86 `recip()`/`rsqrt()` are now exact IEEE division on every width — closes [#64](https://github.com/imazen/archmage/issues/64).** The last estimate-and-refine holdouts: 128/256-bit f32 (`rcpps`/`rsqrtps` + one Newton step, pinned at 4 ULP "pending an x86 run") and the 512-bit `f32x16`/`f64x8` V4/V4x impls (`rcp14`/`rsqrt14` refined, both element types). Refinement computes `a*r = inf*0 = NaN` at `±0`/`±inf` — the #64 sigmoid footgun (`((-v).exp_midp() + one).recip()` NaN'd 97% of a conv tensor) — and misses the 0 ULP contract. The deferred measurement now exists (`benchmarks/recip_x86_zen5-9950x3d_2026-09-03.md`, Zen 5: exact ~1.9x/~3.6x slower than the removed Newton forms in an L1 microbench) and exactness ships anyway: correctness wins in the full-precision tier, `rcp_approx`/`rsqrt_approx` keep the throughput path. `precise_reciprocals.rs` now pins 0 ULP + IEEE rails on every backend and width, 512-bit and V4/V4x included; front-end and backend-trait docs state the division contract and the estimate tier's per-backend rail caveats. (dec6a46)
...
- Third-party requirement bumps: `regex` 1.12.2→1.13.1, `serde` 1.0.228→1.0.229, `toml` 1.1.0→1.1.4, `rustversion` 1.0.22→1.0.23.
- `downstream-compat` fixtures now pin the current published consumers instead of long-stale ones, so the CI gate exercises what users actually build: `linear-srgb` 0.6→0.6.12, `zensim` 0.2→0.2.7, `zenpixels-convert` 0.1→0.2.16. Their committed lockfiles had drifted to `archmage` 0.9.7/0.9.8.
- `zenbench` was held at 0.1.7 in `Cargo.lock` (a plain `cargo update` moved it to 0.1.9); zen-family crates are pinned back after third-party refreshes so they never move as a side effect.

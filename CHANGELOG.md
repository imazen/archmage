...
- **`forge_token_dangerously()` is now a safe `#[target_feature]` function, making rustc the arbiter of every token forge ([#87](https://github.com/imazen/archmage/pull/87)).** Inside a `#[target_feature]` region — an `#[arcane]`, `#[rite]` or `#[magetypes]` body — the attribute already *is* the proof, and the call now needs no `unsafe`: `let token = X64V3Token::forge_token_dangerously();` in a `#[rite(v3)]` body compiles under `#![forbid(unsafe_code)]`. This closes the "rite token forging" open question: a tokenless tier-based `#[rite]` can materialize the token it was never handed, without a redundant `summon().unwrap()` and without generating `unsafe`. From every other context the call is still `unsafe`, source-compatible with existing `unsafe { forge_token_dangerously() }` call sites. rustc rejects forging a stronger token from a weaker context, forging from no context, and coercion to a safe function pointer; features enabled only *globally* (`-C target-feature`, `-C target-cpu=native`) do not count, they must be on the caller's own attribute. Pinned by `tests/forge_from_context.rs` (`#![forbid(unsafe_code)]`) and four `tests/compile_fail/forge_*.rs` cases.
  - No longer `#[deprecated]`, and no longer gated behind `forge-token-api` — a compiler-checked constructor that fires a deprecation warning on every safe use would be unusable, and CI denies warnings. `forge-token-api` becomes a backwards-compat no-op; `tests/token_downcast.rs`, which that gate had been excluding from CI entirely, now runs.
  - **Behaviour unchanged on every hot path.** The generated internals (`summon()`, the cold detect fns, the extraction methods, `IntoConcreteToken`) moved to a new crate-internal `new_unchecked()` that carries no feature gate and stays `#[inline(always)]` — a `#[target_feature]` constructor cannot be `#[inline(always)]` and would have put an optimization boundary in the middle of `summon()`. The generated token code is byte-identical to 0.9.28 outside the constructor definitions themselves.
  - Two API-surface consequences on non-native architectures: stub constructors keep `unsafe fn` (no `#[target_feature]` context for those features can exist there), and `ScalarToken::forge_token_dangerously()` is an ungated safe `fn` (it asserts the empty feature set).

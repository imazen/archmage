...
- magetypes: **CI matrix `f16-version-gate`** that builds + runs the exhaustive `convert_f16` bit-identity test on `aarch64-unknown-linux-gnu` (via `cross`, QEMU `QEMU_CPU=max` so `fp16` is present) across the two toolchains that straddle the boundary, **rustc 1.93 / 1.94** (`fail-fast: false`). This is the safety net that makes the `#[rustversion::since(1.94)]` version bound load-bearing: the 1.93 cell proves the pre-stabilization software fallback compiles with no MSRV violation and tests pass (software-vs-software, 0 NaN divergence); the 1.94 cell proves the gate flips on and the intrinsic resolves. (Normal CI only tests 1.89 + stable, which can't detect an off-by-one in the bound — both sides pass regardless.) Ongoing aarch64 coverage of `convert_f16` at stable already comes from `test-aarch64` + `test-cross`, and on nightly from `miri`, so the gate stays minimal at two cells. The test reads a `rustversion`-gated `neon_f16_hw_compiled()` helper so it expects the HW path's 1022-NaN divergence **only** when the kernel is actually compiled (≥ 1.94), not merely when the CPU reports `fp16`. Existing CI jobs (native ARM `test-aarch64`, `test-cross`, `msrv`, windows-11-arm/macos-intel/i686 coverage) are unchanged (40bd485).

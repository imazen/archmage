...
fn exercise_f32x4<
    T: magetypes::simd::backends::F32x4Backend + magetypes::simd::backends::F32x4Convert,
>(
    token: T,
) {
    for chunk in F32_EDGE.chunks_exact(4) {
        ...
        emit(
            ...
            "round",
            &input_s,
            &fmt_f32_array(&v.round().to_array()),
        );
        emit(
            "f32x4",
            "floor",
            &input_s,
            &fmt_f32_array(&v.floor().to_array()),
        );
        emit(
            "f32x4",
            "ceil",
            &input_s,
            &fmt_f32_array(&v.ceil().to_array()),
        );
        emit(
            "f32x4",
            "sqrt",
            &input_s,
            &fmt_f32_array(&v.sqrt().to_array()),
        );
        emit(
            "f32x4",
            "abs",
            &input_s,
            &fmt_f32_array(&v.abs().to_array()),
        );
    ...
    for (ca, cb) in F32_EDGE.chunks_exact(4).zip(F32_EDGE_B.chunks_exact(4)) {
        ...
        emit(
            ...
            "min",
            &input_s,
            &fmt_f32_array(&va.min(vb).to_array()),
        );
        emit(
            "f32x4",
            "max",
            &input_s,
            &fmt_f32_array(&va.max(vb).to_array()),
        );
    }
...
fn exercise_f32x8<
    T: magetypes::simd::backends::F32x8Backend + magetypes::simd::backends::F32x8Convert,
>(
    token: T,
) {
...
fn exercise_i32x4<T: magetypes::simd::backends::I32x4Backend>(token: T) {
    for chunk in I32_EDGE.chunks_exact(4) {
        ...
        emit(
            ...
            "not",
            &input_s,
            &fmt_i32_array(&v.not().to_array()),
        );
    }

    for (ca, cb) in I32_EDGE
        .chunks_exact(4)
        .zip(I32_EDGE.chunks_exact(4).skip(1))
    {
        ...
        emit(
            ...
            "min",
            &input_s,
            &fmt_i32_array(&va.min(vb).to_array()),
        );
        emit(
            "i32x4",
            "max",
            &input_s,
            &fmt_i32_array(&va.max(vb).to_array()),
        );
    }
}

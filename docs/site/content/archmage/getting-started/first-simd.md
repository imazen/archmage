...
Since Rust 1.86, calling a `#[target_feature]` function from another function with matching features is safe — that's why `square(v)` needs no `unsafe` block inside the `#[arcane]` function. The caller's V3 features are a superset of the callee's V3 features, so the compiler verifies safety at compile time.

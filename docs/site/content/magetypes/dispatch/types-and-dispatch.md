...
When only one tier needs hand-tuning, leave the rest to `#[magetypes]` and add a standalone `#[arcane]` for the special tier. `incant!` will pick it up automatically via the suffix convention:

```rust
use archmage::prelude::*;
use magetypes::simd::generic::f32x16 as GenericF32x16;

// V4/V3/NEON/WASM/scalar via #[magetypes] (from the first section above).
// Now add a v4x specialization using native 512-bit f32x16:
...
The `_v4x` function name matches the suffix `incant!` expects, so the rest of the tier list is served by `#[magetypes]`'s generated variants.

...
let b = i16x8::from_array(token, [9, 10, 11, 12, 13, 14, 15, 16]);
...
// i32x4 -> i16x8
let c = i32x4::from_array(token, [1, 2, 3, 4]);
let d = i32x4::from_array(token, [5, 6, 7, 8]);
...
```rust
#[arcane]
fn brighten(token: Desktop64, pixels: &[u8; 16], amount: i16) -> [u8; 16] {
    let v = i16x8::from_array(token, [
    ...
    let brightness = i16x8::splat(token, amount);

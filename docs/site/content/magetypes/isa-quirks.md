...
| `v.recip()` / `v.rsqrt()` on `f32x4`, `f32x8`; V3 `f32x16` | V3 native half | Same estimate and Newton arithmetic, then unordered compare and blend back to the estimate on invalid intermediates. | Two extra vector operations per native half. Constants, loads and surrounding scheduling affect time. |
| `v.recip()` / `v.rsqrt()` on native `f32x16` | V4/V4x | Estimate + Newton + `VFIXUPIMM` with a constant rail table. | One fixup instruction per vector plus table materialization. |
